...
struct
  let lattice (type a) (q: a t): (module Lattice.S with type t = a) =
    ...
    | MayBeMultiple _ -> (module MayBool)
...
struct
  ...
  let compare a b =
    ...
    if r <> 0 then
      ...
      | Any (MayBeMultiple v1), Any (MayBeMultiple v2) -> CilType.Varinfo.compare v1 v2
