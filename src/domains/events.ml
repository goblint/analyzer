...
type t =
  ...
  | Assert of exp
  | Unassume of {exp: CilType.Exp.t; tokens: WideningTokens0.Token.t list}
...
let pretty () = function
  ...
  | Unassume {exp; tokens} -> dprintf "Unassume {exp=%a; tokens=%a}" d_exp exp (d_list ", " WideningTokens0.Token.pretty) tokens
