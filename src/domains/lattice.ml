...
struct
  ...
  let pp_diff ppf _ = Pretty.text "UnitConf: impossible" ppf
end
...
struct
  ...
  let pp_diff ppf (x,y) =
    Pretty.dprintf "%s: %a not equal %a" (Base.name ()) pp x pp y ppf
end
...
struct
  ...
  let pp_diff ppf _ = Pretty.text "FakeSingleton: impossible" ppf
end
...
struct
  ...
  let pp_diff ppf (x,y) = Base.pp_diff ppf (x.BatHashcons.obj,y.BatHashcons.obj)
end
...
struct
  ...
  let pp_diff ppf ((x:t),(y:t)) = M.pp_diff ppf (unlift x, unlift y)
end
...
struct
  ...
  let pp_diff ppf ((x:t),(y:t)) =
    match (x,y) with
    | (`Lifted x, `Lifted y) -> Base.pp_diff ppf (x,y)
...
struct
  ...
  let pp_diff ppf ((x:t),(y:t)) =
    match x, y with
    | `Lifted1 x, `Lifted1 y -> Base1.pp_diff ppf (x, y)
    | `Lifted2 x, `Lifted2 y -> Base2.pp_diff ppf (x, y)
    | _ when leq x y -> Pretty.text "No Changes" ppf
    | _ -> Pretty.dprintf "%a instead of %a" pp x pp y ppf
...
struct
  ...
  let pp_diff ppf ((x1,x2:t),(y1,y2:t)) =
    if Base1.leq x1 y1 then
      Base2.pp_diff ppf (x2,y2)
    else
      Base1.pp_diff ppf (x1,y1)
...
struct
  ...
  let pp_diff ppf ((x1,x2,x3:t),(y1,y2,y3:t)) =
    if not (Base1.leq x1 y1) then
      Base1.pp_diff ppf (x1,y1)
    else if not (Base2.leq x2 y2) then
      Base2.pp_diff ppf (x2,y2)
    else
      Base3.pp_diff ppf (x3,y3)
...
struct
  ...
  let pp_diff ppf ((x:t),(y:t)) =
    match x, y with
    | `Lifted x, `Lifted y -> Base.pp_diff ppf (x, y)
    | _ -> Pretty.dprintf "%s: %a not leq %a" (name ()) pp x pp y ppf
