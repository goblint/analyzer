...
struct
  let show x = Pretty.sprint ~width:max_int (fun ppf -> P.pp ppf x)
...
struct
  ...
  let pp ppf (state:t) =
    match state with
    | `Left n ->  Base1.pp ppf n
    | `Right n ->  Base2.pp ppf n
