...
let parse ?repl:(repl=false) ?print:(print=false) ?dot:(dot=false) cin =
  ...
  try
    while true do (* loop over all lines *)
      ...
      | e when repl -> Logs.error "%s" (Printexc.to_string e)
    done;
  ...
  | SpecCore.Eof   ->
    ...
    if save_dot && not dot then (
      ...
      Logs.info "saved graph as %s/result/graph.dot" (Sys.getcwd ());
    );
    if dot then (
      print_endline (SpecCore.to_dot_graph !defs) (* nosemgrep: print-not-logging *)
    );
    (nodes, edges)
