...
struct
  ...
  let solve_equation_for_key ((key_to_solve_for, const1), (key2, const2), const) store =
    ...
    let key2_int_dom_tuple, _, _ = (Domain.to_int_val (Store.find key2 store)) in
  ...
  let filter_equations_for_useful_keys filter (store, equations) =
    filter(
      fun ((key1, const1), (key2, const2), const) ->
        not(Domain.is_top (Store.find key1 store)) && not(Domain.is_top (Store.find key1 store))
    ) equations
...
struct
  ...
  let join_equations eqmap1 eqmap2 =
