...
let array_oob_check ( type a ) (module Idx: IntDomain.Z with type t = a) (x, l) (e, v) =
  if GobConfig.get_bool "ana.arrayoob" then (* The purpose of the following 2 lines is to give the user extra info about the array oob *)
    ...
    | Some true, Some false -> (* The following matching differentiates the must and may cases*)
      M.error_each ~warning:(M.Warning.Behavior.Undefined.ArrayOutOfBounds.past_end ()) "Must access array past end"
    | Some true, None ->
      M.warn_each ~warning:(M.Warning.Behavior.Undefined.ArrayOutOfBounds.past_end ()) "May access array past end"
    | Some false, Some true ->
      M.error_each ~warning:(M.Warning.Behavior.Undefined.ArrayOutOfBounds.before_start ()) "Must access array before start"
    | None, Some true ->
      M.warn_each ~warning:(M.Warning.Behavior.Undefined.ArrayOutOfBounds.before_start ()) "May access array before start"
    | _ ->
      M.warn_each ~warning:(M.Warning.Behavior.Undefined.ArrayOutOfBounds.unknown ()) "May access array out of bounds"
  else ()
