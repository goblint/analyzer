...
struct
  ...
  let set (ask: VDQ.t) a (ie, i) v =
    match ie with
    | Some ie when CilType.Exp.equal ie Offset.all_index_exp ->
      v
    | _ ->
      join a v
  ...
  let invariant ~value_invariant ~offset ~lval x =
    ...
    | NoOffset when get_bool "witness.invariant.goblint" ->
      let i_lval = Cil.addOffsetLval (Index (Offset.all_index_exp, NoOffset)) lval in
...
struct
  ...
  let set_with_length length (ask:VDQ.t) x (i,_) a =
    ...
    | Some ie when CilType.Exp.equal ie Offset.all_index_exp ->
      ...
      Joint a
    | Some i when CilType.Exp.equal i Offset.any_index_exp ->
