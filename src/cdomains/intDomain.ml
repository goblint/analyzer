...
struct
  ...
  let widen ik x y =
    ...
    let upper_threshold u =
      let ts = if get_bool "ana.autotune.enabled" then WideningThresholds.upper_thresholds () else ResettableLazy.force widening_thresholds in
    ...
    let lower_threshold l =
      let ts = if get_bool "ana.autotune.enabled" then WideningThresholds.lower_thresholds () else ResettableLazy.force widening_thresholds_desc in
