        ...
        | Addr (v, o) as a ->
          begin try Addr (v, (adjust_offs v o None)) (* cast of one address by adjusting the abstract offset *)
            with CastError s -> (* don't know how to handle this cast :( *)
              ...
              (*raise (CastError s)*)
            | SizeOfError (s,t) ->
              M.warn_each @@ M.Unknown ("size of error: " ^  s);
              a
          end
    ...
    let rec do_eval_offset (ask:Q.ask) f (x:t) (offs:offs) (exp:exp option) (l:lval option) (o:offset option) (v:lval option) (t:typ): t =
      ...
      | _ ->
        ...
        | `Field (fld, offs) when fld.fcomp.cstruct -> begin
            ...
            | `Address ad when fld.fcomp.cname = "list_head" || fld.fname = "next" || fld.fname = "prev" ->
              ...
              begin match f ad with
                ...
                | _ -> M.warn @@ M.Unknown "Trying to read a field, but was not given a struct"; top ()
              end
            | `Struct str ->
        ...
        | `Field (fld, offs) -> begin
            ...
            | _ -> M.warn @@ M.Unknown "Trying to read a field, but was not given a union"; top ()
          end
        | `Index (idx, offs) -> begin
            ...
            | _ -> M.warn (M.Unknown ("Trying to read an index, but was not given an array ("^show x^")")); top ()
          end
    in
    ...
    let rec do_update_offset (ask:Q.ask) (x:t) (offs:offs) (value:t) (exp:exp option) (l:lval option) (o:offset option) (v:lval) (t:typ):t =
      ...
      let result =
        ...
        | `Field (fld, offs) when fld.fcomp.cstruct -> begin
            ...
            | `Top -> M.warn @@ M.Unknown "Trying to update a field, but the struct is unknown"; top ()
            | _ -> M.warn @@ M.Unknown "Trying to update a field, but was not given a struct"; top ()
          end
        | `Field (fld, offs) -> begin
            let t = fld.ftype in
            let l', o' = shift_one_over l o in
            ...
            | `Union (last_fld, prev_val) ->
              let tempval, tempoffs =
                if UnionDomain.Field.equal last_fld (`Lifted fld) then
                  ...
                  | _ -> M.warn_each @@ M.Unknown "Why are you indexing on a union? Normal people give a field name.";
                    top (), offs
                end
              in
            ...
            | `Top -> M.warn @@ M.Unknown "Trying to update a field, but the union is unknown"; top ()
            | _ -> M.warn_each @@ M.Unknown "Trying to update a field, but was not given a union"; top ()
          end
        | `Index (idx, offs) -> begin
            let l', o' = shift_one_over l o in
            match x with
            ...
            | `Bot ->
              ...
              let new_array_value =  CArrays.set ask x' (e, idx) new_value_at_index in
              `Array new_array_value
            | `Top -> M.warn @@ M.Unknown "Trying to update an index, but the array is unknown"; top ()
            | x when Goblintutil.opt_predicate (BI.equal BI.zero) (IndexDomain.to_int idx) -> do_update_offset ask x offs value exp l' o' v t
            | _ -> M.warn_each (M.Unknown ("Trying to update an index, but was not given an array("^show x^")")); top ()
          end
      in mu result
    in
    let l, o = match exp with
