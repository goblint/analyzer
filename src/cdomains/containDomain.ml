...
(*
    ...
    let dbg_report x =
        ...
        Messages.warn ~msg:((*(string_of_int !counter)^*)"CW: "^x) ()
  ...
  let danger_propagate v args (fd,st,(gd:Diff.t)) must_assign fs glob comm = (*checks if the new danger val points to this->something and updates this->something*)
      ...
      let (fd,st,gd) = if (may_be_constructed_from_this st (Lval (Var (FieldVars.get_var fv),NoOffset))) then
          begin
            let rhs_cft = ArgSet.fold (fun x y->
                ...
                then
                  begin
                    (*Messages.warn ~msg:((string_of_int (!uid) )^":"^(sprint 160 (FieldVars.pretty () x))^" it "^string_of_bool it^" cft "^string_of_bool cft) ();*)
                    false
                  end
                else y)
                args true
            in
            (*Messages.warn ~msg:((string_of_int (!uid) )^":"^"danger.UPDATE_THIS_2 "^v.vname^" -> "^(sprint 160 (FieldVars.pretty () fv))^" = "^sprint 160 (ArgSet.pretty () args)^" rhs_ctf : "^string_of_bool rhs_cft) ();*)
    ...
    let (fd,st,gd) =
      if (*not must_assign && MUST PROPAGATE HERE!!!*)not (ArgSet.is_bot ds) then
        begin
          (*Messages.warn ~msg:((string_of_int (!uid) )^":"^"danger.prop_ds_1 "^v.vname^" -> "^sprint 160 (ArgSet.pretty () ds)^" = "^sprint 160 (ArgSet.pretty () args)) ();*)
            ...
            begin
              let no_vtbl =
                ...
                begin
                  ...
                  begin
                    (*muid := !muid +1;
                      Messages.warn ~msg:((string_of_int !muid)^":no_vtbl2t : "^(sprint 160 (d_lval () lval))^ " = "^sprint 160 (ArgSet.pretty () ns)) ();*)
