...
struct
      ...
      | StartOf (Var v2,o) -> 
          ...
          if ArgSet.is_top x then begin
            ...
            used_args_idx o
					...
					begin
						(* report ("used args" ^v2.vname^":"^sprint 80 (ArgSet.pretty () x)^"\n"); *)
    ...
    let rec from_this = function 
      ...
      | StartOf (Var v2,o) -> 
          ...
          let res2=not (ArgSet.is_bot x) && (ArgSet.for_all (fun v -> (FieldVars.get_var v).vname = this_name) x)
					...
					(*if (ArgSet.is_bot x) then report ("bot_args: " ^(sprint 80 (d_exp () e)));
					report ("cft: " ^(sprint 80 (d_exp () e))^" name : "^v2.vname^" as : "^(sprint 80 (ArgSet.pretty () x))^"\n");*)
  ...
  let is_safe e = (*check exp*)
    ...
    let danger_propagate v args (fd,st,gd) = (*checks if the new danger val points to this->something and updates this->something*)
		  (*printf "%s\n" ("danger.prop "^v.vname^" = "^sprint 80 (ArgSet.pretty () args));*)
				...
				begin
					...
					let update_this fv (fd,st,gd) = 
						...
						begin
							...
							in 
              (*report ("update_this: " ^v.vname^" "^sprint 80 (FieldSet.pretty () field));*)
							(fd,st, Diff.add (tainted_varinfo (), field)  gd) (*update this->field?*)
							else
							begin
  ...
  let may_be_a_perfectly_normal_global ask e fromFun (fn,st,gd) fs = 
    ...
    begin
			(*report ("mbg_start: " ^(sprint 80 (d_exp () e)));*)
    ...
    in*)		 		  
      ...
      in
			(*report ("mbg: " ^(sprint 80 (d_exp () e))^ "\tlocal : "^(string_of_bool is_local)^"\tdanger : "^string_of_bool is_danger^"\n");*)
  ...
  let assign_to_lval ask lval (fd,st,gd) args = (*propagate dangerous vals*)
    match lval with 
      | Var v , ofs -> (*report ("danger.add v"^v.vname^" = "^sprint 80 (ArgSet.pretty () args));*)
      ...
      | Mem e , ofs -> (*if it's either not const from this or has no fields*)
			    ...
			    let cft = (constructed_from_this st (Lval lval)) in
					let fse = (FieldSet.is_bot (get_field_from_this (Lval lval))) in 
					(*report ((sprint 80 (d_lval () lval))^" cft "^(string_of_bool cft)^" fse "^(string_of_bool fse));*)         
					if  (cft && fse) || (not cft) then 
					begin                              
						let vars = get_vars e in (*not very exact for huge compount statements*)
						List.fold_right 
	            (fun x y->(*report ("danger.add e"^x.vname^" = "^sprint 80 (ArgSet.pretty () args));*)
    ...
    match ask (Queries.MayPointTo e) with
      ...
      | `LvalSet s when not (Queries.LS.is_top s) ->
          let add_lv (v,_) (fd,st,gd) = 
						(*report ("danger.add "^v.vname^" = "^sprint 80 (ArgSet.pretty () args));*)
            danger_propagate v args (fd,st,gd)
          in
  ...
  let assign_argmap ask lval exp (fd, st, df) = (*keep track of used fun args*)
          ...
          | s when ArgSet.is_bot s -> let vars= get_vars exp in 
              let s = List.fold_right (fun x y->if not (is_safe_name x.vname) then begin ArgSet.add (FieldVars.gen x) y end else y) vars (ArgSet.empty()) in
							(*report ("assign_argmap :no ags: " ^(sprint 80 (d_lval () lval))^ " = "^(sprint 80 (d_exp () exp))^":"^sprint 80 (ArgSet.pretty () s)^"\n");*)
							if not (ArgSet.is_bot s) then					
					    assign_to_lval ask lval (fd, st, df) s
							else 
							(fd, st, df)
          | s -> 
						 (*report ("assign_argmap :assign: " ^(sprint 80 (d_lval () lval))^ " = "^(sprint 80 (d_exp () exp))^":"^sprint 80 (ArgSet.pretty () s)^"\n");*)
					   begin							  
					      let fs = get_field_from_this exp in
							  let (fd, st, df) = if FieldSet.is_bot fs || not (constructed_from_this st exp) then 
								...
								begin
									let merge v fs (fd, st, df) = (*propagate the fields*)
										...
										(*report (sprint 80 (ArgSet.pretty () s));*)
										assign_to_lval ask lval (fd, st, df) s
									in
    ...
    then begin		
			(*
			 let str=match rval with 
				| Some rval -> 	 sprint 80 (d_exp () rval)
				| _ -> "??"
			in
			report ("Fields "^sprint 80 (FieldSet.pretty () flds)^" tainted with"^str);*)  
  ...
  let add_htbl_entry ht c n =
	...
	let add_func_ptr f = (*move fun to pub (also stays in priv) FIXME: side efect!*)
  ...
  let warn_tainted fs (_,ds,_) (e:exp) =
		...
		(*report (sprint 80 (d_exp () e)^" cft: "^string_of_bool cft^" it: "^string_of_bool it);*) 
    if cft && it then
      if not (ArgSet.is_bot dargs)    
      then begin
          report (" (3) Use of tainted field found in " ^ sprint 80 (d_exp () e)^" which may point to "^ArgSet.short 80 dargs^".");
      end
			else			
        report (" (3) Use of tainted field found in " ^ sprint 80 (d_exp () e))    											
