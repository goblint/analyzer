...
struct
  ...
  let copy x = x
  type t = fieldinfo
  let compare x y = compare (x.fname,x.fcomp.cname) (y.fname,y.fcomp.cname)
  let equal x y = x.fcomp.cname = y.fcomp.cname && x.fname = y.fname
  let hash x = Hashtbl.hash (x.fname, x.fcomp.cname)
