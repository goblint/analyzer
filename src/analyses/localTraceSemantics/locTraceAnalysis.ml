...
struct
  ...
  let rec create_local_assignments graphList globals ctx =
    let rec loop global graphList acc =
      ...
      |  graph::gs ->
        ...
        let localGlobalVar = customVinfoStore#getLocalVarinfo (make_custom_local_varinfo_name global) global.vtype in
        ...
        let customMutex = customVinfoStore#getGlobalVarinfo (make_mutex_varinfo_name global) in
        let lockingLabel:Edge.t = Proc(None, Lval(Var(lockVarinfo), NoOffset),[AddrOf(Var(customMutex), NoOffset)]) in
        let lvalLockedLs = VarinfoSet.add customMutex lastNode.lockSet in
        let lockedNode = {programPoint=lastNode.programPoint;sigma=lastNode.sigma; id=(idGenerator#getID lastNode (EdgeImpl.convert_edge lockingLabel) lastNode.programPoint lastNode.sigma lastNode.tid lvalLockedLs);tid=lastNode.tid;lockSet=lvalLockedLs} in
        ...
        let firstNode = LocalTrace.get_first_node graph in
        let firstLockEdge:node*CustomEdge.t*node =(firstNode, DepMutex(customMutex),lockedNode) in
        let lockedGraph = LocalTrace.extend_by_gEdge graph lockingEdge in
        let firstLockedGraph = if (NodeImpl.equal firstNode lastNode) then lockedGraph
          ...
          else lockedGraph (* This case happens if lock(m) is executed consecutively. Should I support that? *) in
        ...
        let graphList = List.fold (fun resultGraphList lockedGraph ->
            let sigmaList,success_inner, _ = eval_wrapper lockedNode.sigma localGlobalVar (Lval(Var(global),NoOffset)) lockedGraph lockedNode true in
            (List.fold (fun sigma_graphList evaluated ->
                 let assignedNode = {programPoint=ctx.prev_node;sigma=evaluated;id=(idGenerator#getID lockedNode
                                                                                      (Assign((Var(localGlobalVar),NoOffset),Lval(Var(global), NoOffset))) ctx.prev_node evaluated lastNode.tid lvalLockedLs);tid=lastNode.tid;lockSet=lvalLockedLs} in
                 let assigningEdge:(node * CustomEdge.t * node) =  (lockedNode, (Assign((Var(localGlobalVar),NoOffset),Lval(Var(global), NoOffset))), assignedNode) in
                 ...
                 let unlockedNode = {programPoint=ctx.prev_node;sigma=evaluated;id=(idGenerator#getID assignedNode unlockingLabel ctx.prev_node evaluated lastNode.tid lastNode.lockSet);tid=lastNode.tid;lockSet=lastNode.lockSet} in
  ...
  let startstate v = let g = D.empty () in 
  ...
  let assign_fold_graphSet graph (lval, rval, ctx,set_acc) =
    let lastNode =
      ...
      else LocalTrace.get_last_node_progPoint graph ctx.prev_node in
    let new_set =
    ...
    "^(LocalTrace.show graph)^"\n"); set_acc) else
         let result_graphList =
           assign_on_node (if LocTraceGraph.is_empty graph then (LocTraceGraph.add_vertex graph lastNode) else graph) ctx lval rval lastNode in
  ...
  let assign ctx (lval:lval) (rval:exp) : D.t =
    ...
    let _, _, _, result =
      D.fold assign_fold_graphSet ctx.local (lval, rval, ctx, D.empty ()) in 
    result
  ...
  let branch_on_node graph ctx exp tv {programPoint=programPoint;id=id;sigma=sigma;tid=tid;lockSet=ls} =
    let rvalGlobals = get_all_globals exp VarinfoSet.empty in 
    print_string ("in branch, rvalGlobals =["^(NodeImpl.show_lockSet rvalGlobals)^"]\n");
    let someTmp = create_local_assignments [graph] (VarinfoSet.to_list rvalGlobals) ctx in
    List.fold (fun resultList_outter graph_outter ->
        (
          let lastNode = LocalTrace.get_last_node graph_outter in
          let branch_sigma = SigmaMap.add LocalTrace.branch_vinfo Error lastNode.sigma in
          let sigmaList,success, newExp = eval_wrapper branch_sigma LocalTrace.branch_vinfo exp graph_outter lastNode tv in
          List.map (fun sigma_map ->
              let result_as_int = match (SigmaMap.find_default Error LocalTrace.branch_vinfo sigma_map) with
                ...
                |_ -> -1 in
              let sigmaNew = remove_global_locals_sigma (SigmaMap.remove LocalTrace.branch_vinfo (NodeImpl.destruct_add_sigma lastNode.sigma sigma_map)) (VarinfoSet.to_list rvalGlobals) in
              print_string ("result_as_int: "^(string_of_int result_as_int)^"\n");
              let myEdge:node*CustomEdge.t*node = (lastNode, Test(newExp, tv),
                                                   {programPoint=ctx.node;sigma=sigmaNew;id=(idGenerator#getID lastNode (Test(newExp, tv)) ctx.node sigmaNew tid ls);tid=tid;lockSet=ls}) in
  ...
  let branch_fold_graphSet graph (exp, tv, ctx,set_acc) =
    let lastNode = LocalTrace.get_last_node_progPoint graph ctx.prev_node in
    let new_set =
  ...
  "^(LocalTrace.show graph)^"\n"); set_acc) else
         let result_graphList =
           branch_on_node graph ctx exp tv lastNode in
  ...
  let branch ctx (exp:exp) (tv:bool) : D.t =
    ...
    let _, _, _, result =
      D.fold branch_fold_graphSet ctx.local (exp, tv, ctx, D.empty ()) in 
    result
  ...
  let body_on_node graph ctx {programPoint=programPoint;id=id;sigma=sigma;tid=tid;lockSet=ls} =
    let myEdge = ({programPoint=programPoint;sigma=sigma;id=id;tid=tid;lockSet=ls},(EdgeImpl.convert_edge ctx.edge),{programPoint=ctx.node;sigma=sigma;id=(idGenerator#getID {programPoint=programPoint;sigma=sigma;id=id;tid=tid;lockSet=ls} (EdgeImpl.convert_edge ctx.edge) ctx.node sigma tid ls);tid=tid;lockSet=ls}) in
  ...
  let body_fold_graphSet graph (ctx,set_acc) =
    ...
    "^(LocalTrace.show graph)^"\n");if LocTraceGraph.is_empty graph
                                                                                   then(
                                                                                     let first_ID = idGenerator#increment() in
                                                                                     let second_ID = idGenerator#increment() in
                     ...
                     let result_graph =
                       body_on_node graph ctx lastNode in
                     D.add result_graph set_acc) in
    (ctx, new_set)
  ...
  let body ctx (f:fundec) : D.t =
    ...
    let _, result =
      D.fold body_fold_graphSet ctx.local (ctx, D.empty ()) in
  ...
  let return_on_node graph ctx exp {programPoint=programPoint;id=id;sigma=sigma;tid=tid;lockSet=ls} (f:fundec) =
    let result_graphList =
      ...
      | None ->  let myEdge =  print_string "In return case None\n";
                   ({programPoint=programPoint;sigma=sigma;id=id;tid=tid;lockSet=ls},(EdgeImpl.convert_edge ctx.edge),{programPoint=ctx.node;sigma=sigma;id=(idGenerator#getID {programPoint=programPoint;sigma=sigma;id=id;tid=tid;lockSet=ls} (EdgeImpl.convert_edge ctx.edge) ctx.node sigma tid ls);tid=tid;lockSet=ls}) in
        let result_graph =LocalTrace.extend_by_gEdge graph myEdge in
        (
      ...
      | Some(ret_exp) -> (  ( print_string ("return expression: "^(CilType.Exp.show ret_exp)^"\n");
                              ...
                              | CastE(TPtr(TVoid(_), attrList2),Const(CInt(cilint,IInt,_))) ->
                                ...
                                [result_graph]
                              | _ ->
                                let rvalGlobals = get_all_globals ret_exp VarinfoSet.empty in 
                                print_string ("in return, rvalGlobals =["^(NodeImpl.show_lockSet rvalGlobals)^"]\n");
                                let someTmp = create_local_assignments [graph] (VarinfoSet.to_list rvalGlobals) ctx in
                                let outterResultList = List.fold ( fun resultList_outter graph_outter ->
                                    (
                                      let lastNode = LocalTrace.get_last_node graph_outter in
                                      let resultList, success, newExp = eval_wrapper lastNode.sigma LocalTrace.return_vinfo ret_exp graph_outter lastNode true in
                                      List.map (fun sigma_map ->
  ...
  "^(LocalTrace.show graph)^"\n"); set_acc) else
        let result_graphList =
          return_on_node graph ctx exp lastNode f in
        List.fold (fun set_fold result_graph -> D.add result_graph set_fold) set_acc result_graphList in
    (f, exp, ctx, new_set)
  ...
  let return ctx (exp:exp option) (f:fundec) : D.t =
    ...
    let _, _, _, result = D.fold return_fold_graphSet ctx.local (f, exp, ctx, D.empty ()) in
    result
  ...
  let special_on_node graph ctx {programPoint=programPoint;id=id;sigma=sigma;tid=tid;lockSet=ls} f arglist lval =
    ...
    | Lock {lock = lockExp;_}, _ ->
      ( let mutex_vinfo =
        ...
        let myTmp:V.t = Mutex(mutex_vinfo) in
        let allUnlockingTraces = ctx.global myTmp in
        ...
        let lockedNode = {programPoint=ctx.node;sigma=sigma;id=(idGenerator#getID {programPoint=programPoint;sigma=sigma;id=id;tid=tid;lockSet=ls} (EdgeImpl.convert_edge ctx.edge) ctx.node sigma tid (VarinfoSet.add mutex_vinfo ls));tid=tid;lockSet=(VarinfoSet.add mutex_vinfo ls)} in
        let firstLockEdge:node*CustomEdge.t*node =(firstNode, DepMutex(mutex_vinfo),lockedNode) in
        let lockingEdge = ({programPoint=programPoint;sigma=sigma;id=id;tid=tid;lockSet=ls},(EdgeImpl.convert_edge ctx.edge),lockedNode) in
        let lockedGraph = LocalTrace.extend_by_gEdge graph lockingEdge in
    ...
    | ThreadJoin { thread = tidExp; ret_var }, _ ->
      let myTmp = (print_string ("We found a pthread_join in special with tidExp="^(CilType.Exp.show tidExp)^" and ret_var="^(CilType.Exp.show ret_var)^"\n");
                   let special_varinfo = makeVarinfo false "__goblint__traces__special" (TInt(IInt,[])) in
                   let tidSigmaList, success, newExp = eval_wrapper sigma special_varinfo tidExp graph {programPoint=programPoint;id=id;sigma=sigma;tid=tid;lockSet=ls} true in 
                   if not success then (Printf.printf "Error: could not evaluate argument of pthread_join in special\n"; exit 0);
                   List.fold (fun graphList tidSigma ->
                       let tidJoin =
                         match SigmaMap.find special_varinfo tidSigma with
                           Int(l,u,_) -> if l = u then Big_int_Z.int_of_big_int l else (Printf.printf "Intervals for pthread_join is not supported in special\n"; exit 0)
                       ...
                       if LocalTrace.is_already_joined tidJoin graph then (
                         ...
                         let endingTraces = graphSet_to_list (ctx.global myTmp) in
                         ...
                         if not (tidRecord#existsTID tidJoin) then (
                           ...
                           (
                             ...
                             List.fold (
                               fun list_fold trace_fold -> let tmp_graph = LocalTrace.merge_graphs graph trace_fold in
                                 let newVarinfoSet = VarinfoSet.union ls (LocalTrace.get_last_node trace_fold).lockSet in
                                 let destination_node = {programPoint=ctx.node;sigma=sigma;id=(idGenerator#getID {programPoint=programPoint;sigma=sigma;id=id;tid=tid;lockSet=ls} (EdgeImpl.convert_edge ctx.edge) ctx.node sigma tid newVarinfoSet);tid=tid;lockSet=newVarinfoSet} in
                                 print_string ("In ThreadJoin, destination_node="^(NodeImpl.show destination_node)^"\n");
                                 let tmp_graph_edge1 = LocalTrace.extend_by_gEdge tmp_graph (LocalTrace.get_last_node trace_fold, EdgeImpl.convert_edge ctx.edge,destination_node) in
                                 let tmp_graph_edge2 = LocalTrace.extend_by_gEdge tmp_graph_edge1 ({programPoint=programPoint;sigma=sigma;id=id;tid=tid;lockSet=ls}, EdgeImpl.convert_edge ctx.edge,destination_node) in
...
"^(List.fold (fun s_fold g_fold -> (LocalTrace.show g_fold)^";\n"^s_fold) "" myTmp)^" \n");
    ...
    with arglist: "^(List.fold (fun acc_fold arg_fold -> (CilType.Exp.show arg_fold)^"; "^acc_fold) "" arglist)^"\n");
        ...
        | Some(Var(var), NoOffset) ->
          let newSigma = SigmaMap.remove var sigma in
          let myEdge = ({programPoint=programPoint;sigma=sigma;id=id;tid=tid;lockSet=ls}, EdgeImpl.convert_edge ctx.edge,
                        {programPoint=ctx.node;sigma=newSigma;id=(idGenerator#getID {programPoint=programPoint;sigma=sigma;id=id;tid=tid;lockSet=ls} (EdgeImpl.convert_edge ctx.edge) ctx.node newSigma tid ls);tid=tid;lockSet=ls}) in
          let result_graph = LocalTrace.extend_by_gEdge graph myEdge in
          [result_graph]
  ...
  let special_fold_graphSet graph (lval, f, arglist, ctx,set_acc) =
    ...
    "^(LocalTrace.show graph)^"\n"); set_acc) else
        let result_graph =
          special_on_node graph ctx lastNode f arglist lval in
        List.fold (fun set_fold graph_fold -> D.add graph_fold set_fold) set_acc result_graph in
  ...
  let special ctx (lval: lval option) (f:varinfo) (arglist:exp list) : D.t =
    ...
    let _, _, _, _, result =   D.fold special_fold_graphSet ctx.local (lval, f, arglist, ctx, D.empty ()) in 
    result
  ...
  "^(LocalTrace.show graph)^"\n"); set_acc) else
        let result_graphList =
          enter_on_node graph ctx f args lastNode in
        List.fold (fun set_fold result_graph -> D.add result_graph set_fold) set_acc result_graphList in
    (f, args,ctx, new_set)
  ...
  with formals "^(List.fold (fun s sformal -> s^", "^(CilType.Varinfo.show sformal)) "" f.sformals)^" and arguments "^(List.fold (fun s exp -> s^", "^(CilType.Exp.show exp)) "" args)^"\n");
    if D.is_empty ctx.local then (
      ...
      let _, _, _, result = print_string ("In enter, neuer state wird erstellt\n mit ctx.local: "^(D.show ctx.local)^" und |ctx.local| = "^(string_of_int (D.cardinal ctx.local))^"\n");
        D.fold enter_fold_graphSet ctx.local (f, args, ctx, D.empty ()) in
      [ctx.local, result]
  ...
  let combine_on_node args ctx {programPoint=programPoint;id=id;sigma=sigma;tid=tid;lockSet=ls} {programPoint=progP_returning;id=id_returning;sigma=sigma_returning;tid=tid_returning;lockSet=ls_returning} callee_local lval graph =
    ...
    let newSigma = remove_global_locals_sigma sigma (VarinfoSet.to_list rvalGlobals) in
         ...
         {programPoint=ctx.node;sigma=newSigma; id=(idGenerator#getID {programPoint=programPoint;sigma=newSigma;id=id;tid=tid_returning; lockSet=ls} (EdgeImpl.convert_edge ctx.edge) ctx.node newSigma tid_returning (VarinfoSet.union ls_returning ls));tid=tid_returning;lockSet=(VarinfoSet.union ls_returning ls)}
                     |Some (Var x, y) ->  if x.vglob
                       then
                         ...
                         (let return_value = SigmaMap.find LocalTrace.return_vinfo sigma_returning in 
                          if equal_varDomain return_value Error then (print_string "In combine, a returning Error is assigned to some lval, this is not supported\n";exit 0) else
                            let result_sigma = SigmaMap.add x return_value newSigma in 
                            {programPoint=progP_returning;sigma=sigma_returning;id=id_returning;tid=tid_returning;lockSet=ls_returning},Skip,{programPoint=ctx.node;sigma=result_sigma; id=(idGenerator#getID {programPoint=programPoint;sigma=newSigma;id=id;tid=tid_returning;lockSet=ls} (EdgeImpl.convert_edge ctx.edge) ctx.node result_sigma tid_returning (VarinfoSet.union ls_returning ls));tid=tid_returning; lockSet=(VarinfoSet.union ls_returning ls)})
  ...
  let combine_fold_graphSet graph (args, lval, callee_local, ctx,set_acc) =
    ...
    let lastGraphNode = LocalTrace.get_last_node graph in 
    if Node.equal lastGraphNode.programPoint LocalTrace.error_node then (print_string "In combine, we have an empty graph\n"; (args, lval, callee_local, ctx,set_acc))
    else (
      let currentNode =
        LocalTrace.find_calling_node lastGraphNode graph ctx.prev_node in
      let new_set =
      ...
      "^(LocalTrace.show graph)^"\n"); set_acc) else
           let result_graph=
             combine_on_node args ctx currentNode lastGraphNode callee_local lval graph in
           D.add result_graph set_acc) in
      (args, lval, callee_local, ctx, new_set))

  let combine_env ctx (lval:lval option) fexp (f:fundec) (args:exp list) fc (callee_local:D.t) f_ask : D.t =
    predominatorRegistration#update ctx.prev_node ctx.node;
    ...
    let _,_,_,_, result=
      D.fold combine_fold_graphSet callee_local (args, lval, callee_local, ctx, D.empty ()) in 
    result

  let combine_assign ctx (lval:lval option) fexp (f:fundec) (args:exp list) fc (callee_local:D.t) f_ask : D.t = ctx.local
      ...
      ( let sigma_formals, _ =
          List.fold (
            fun (sigAcc, formalExp) formal -> (match formalExp with
                ...
                | [] -> Printf.printf "Fatal error: missing expression for formals in enter\n"; exit 0)
          ) (sigma, args) fd.sformals in 
        print_string ("sigma_formals: "^(NodeImpl.show_sigma sigma_formals)^"\n");
        let new_id = idGenerator#getID {programPoint=programPoint;sigma=sigma;id=id;tid=tid;lockSet=ls} (EdgeImpl.convert_edge ctx.edge) ctx.node sigma_formals id VarinfoSet.empty in
        (* create dependency is just the create edge itself *)
        let myEdge = {programPoint=programPoint;sigma=sigma;id=id;tid=tid;lockSet=ls}, EdgeImpl.convert_edge ctx.edge,{programPoint=(FunctionEntry(fd));sigma=sigma_formals;id=new_id;tid=id;lockSet=VarinfoSet.empty} in
        let result_graph = print_string ("In threadenter, we add the edge "^(LocalTrace.show_edge myEdge)^"\n
  to the graph:"^(LocalTrace.show graph)^"\n"); LocalTrace.extend_by_gEdge graph myEdge in
        result_graph
      )
  ...
  let threadenter_fold_graphSet graph (args, f, ctx,set_acc) =
   ...
   "^(LocalTrace.show graph)^"\n"); set_acc) else(
        let result_graph = threadenter_on_node graph ctx f args lastNode in
        D.add result_graph set_acc) in
    (args, f, ctx, new_set)
  ...
  let threadenter ctx lval f (args:exp list) =
    ...
    let _, _, _, result = D.fold threadenter_fold_graphSet (ctx.local) (args, f, ctx, D.empty()) in
    [result]
        ...
        {programPoint=programPoint;sigma=sigma;id=id;tid=tid;lockSet=ls}, EdgeImpl.convert_edge ctx.edge,{programPoint=ctx.node;sigma=sigma;id=(idGenerator#getID {programPoint=programPoint;sigma=sigma;id=id;tid=tid;lockSet=ls} (EdgeImpl.convert_edge ctx.edge) ctx.node sigma tid ls);tid=tid;lockSet=ls}
                                 | Some(Mem(CastE(TPtr(TNamed(tInfo, tAttr), ptrAttr), AddrOf(Var(lvalVinfo),_))), offset) -> (
                                     ...
                                     then (print_string ("input in threadspawn is pthread_t\n");
                                           let result_sigma = SigmaMap.add lvalVinfo (ThreadID(id)) sigma in
  ...
  let threadspawn_fold_graphSet graph (lval, ctx,set_acc) =
    ...
    let new_set =
     ...
     "^(LocalTrace.show graph)^"\n"); set_acc) else(
        let result_graph = threadspawn_on_node graph ctx lval lastNode in
        D.add result_graph set_acc) in
    (lval, ctx, new_set)
  ...
  let threadspawn ctx lval f args fctx =
    ...
    let _, _, result = D.fold threadspawn_fold_graphSet (ctx.local) (lval, ctx, D.empty()) in
    result
end
