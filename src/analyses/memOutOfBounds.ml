...
struct
  ...
  let eval_ptr_offset_in_binop man exp ptr_contents_typ =
    ...
    | `Lifted eo ->
      let casted_eo = ID.cast_to ~kind:Unknown (Cilfacade.ptrdiff_ikind ()) eo in (* TODO: proper castkind *)
      begin
    ...
    let rec convert_offset (ofs: offset) =
      ...
      | Index (exp, ofs) ->
        let i = match man.ask (Queries.EvalInt exp) with
          | `Lifted x -> ID.cast_to ~kind:Unknown (Cilfacade.ptrdiff_ikind ()) x (* TODO: proper castkind *)
    ...
    if (not @@ lval_contains_a_ptr lval) || ptr_only_has_str_addr man (Lval lval) then ()
      ...
      | (Mem e, o), _ ->
        ...
        begin match e_size with
          ...
          | `Lifted es ->
            let casted_es = ID.cast_to ~kind:Unknown (Cilfacade.ptrdiff_ikind ()) es in (* TODO: proper castkind *)
            let casted_offs = ID.cast_to ~kind:Unknown (Cilfacade.ptrdiff_ikind ()) offs_intdom in (* TODO: proper castkind *)
  ...
  and check_no_binop_deref man lval_exp =
    ...
    | Some t ->
      begin match ptr_size, addr_offs with
        ...
        | `Lifted ps, ao ->
          let casted_ps = ID.cast_to ~kind:Unknown (Cilfacade.ptrdiff_ikind ()) ps in (* TODO: proper castkind *)
          let casted_ao = ID.cast_to ~kind:Unknown (Cilfacade.ptrdiff_ikind ()) ao in (* TODO: proper castkind *)
      ...
      begin match ptr_contents_type with
        | Some t ->
          ...
          let offset_size_with_addr_size = match offset_size with
            | `Lifted os ->
              let casted_os = ID.cast_to ~kind:Unknown (Cilfacade.ptrdiff_ikind ()) os in (* TODO: proper castkind *)
              let casted_ao = ID.cast_to ~kind:Unknown (Cilfacade.ptrdiff_ikind ()) addr_offs in (* TODO: proper castkind *)
              begin
          ...
          begin match ptr_size, offset_size_with_addr_size with
            ...
            | `Lifted ps, `Lifted o ->
              let casted_ps = ID.cast_to ~kind:Unknown (Cilfacade.ptrdiff_ikind ()) ps in (* TODO: proper castkind *)
              let casted_o = ID.cast_to ~kind:Unknown (Cilfacade.ptrdiff_ikind ()) o in (* TODO: proper castkind *)
  ...
  let check_count man fun_name ptr n =
    ...
    | `Lifted ds, `Lifted en ->
      ...
      let casted_en = ID.cast_to ~kind:Unknown (Cilfacade.ptrdiff_ikind ()) en in (* TODO: proper castkind *)
      let casted_ao = ID.cast_to ~kind:Unknown (Cilfacade.ptrdiff_ikind ()) addr_offs in (* TODO: proper castkind *)
