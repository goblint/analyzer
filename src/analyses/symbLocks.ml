...
struct
  ...
  let special ctx lval f arglist =
    ...
    | Unknown, fn when VarEq.safe_fn fn ->
      Messages.warn ~category:Unsound "Analyzer assuming that %s does not change lockset." fn;
      ctx.local
    | _, _ ->
      let st =
        match lval with
    ...
    *)
    ...
    let one_lockstep (_,a,m) xs =
      ...
      | AddrOf (Var v,o) ->
        ...
        A.add (`Right lock) xs
      | _ ->
        Messages.warn ~category:Unsound "Internal error: found a strange lockstep pattern.";
        xs
    in
    let do_perel e xs =
