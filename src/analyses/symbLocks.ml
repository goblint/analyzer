...
struct
  ...
  let special ctx lval f arglist =
    ...
    | `Unknown fn when VarEq.safe_fn fn ->
      Messages.warn (Messages.LogEvent.may (Messages.EventType.Unknown (("Assume that "^fn^" does not change lockset."))));
      ctx.local
    | `Unknown x -> begin
        let st =
          match lval with
  ...
  let add_per_element_access ctx e rw =
    ...
    *)
    ...
    let one_lockstep (_,a,m) xs =
      ...
      | _ ->
        Messages.warn (Messages.LogEvent.may (Messages.EventType.Unknown ("Internal error: found a strange lockstep pattern.")));
        xs
    in
    let do_perel e xs =
