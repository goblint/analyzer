        ...
        | Lval rlval -> begin
            match ask (Queries.MayPointTo (mkAddrOf rlval)) with
              | rv when not (Queries.LS.is_top rv) && Queries.LS.cardinal rv = 1 ->
                  let rv = Lval.Exp.to_cil_exp (Queries.LS.choose rv) in
  ...
  let combine_env ctx lval fexp f args fc au (f_ask: Queries.ask) =
    ...
    let d_local =
      ...
      if Queries.LS.is_top tainted || not (ctx.ask (Queries.MustBeSingleThreaded {since_start = true})) then
        ...
        let taint_exp = Queries.ES.of_list (List.map Lval.Exp.to_cil_exp (Queries.LS.elements tainted)) in
  ...
  let remove_reachable ~deep ask es st =
    ...
    | Some rs ->
      ...
      Queries.LS.fold (fun lval st ->
          remove ask (Lval.Exp.to_cil lval) st
        ) rs st
