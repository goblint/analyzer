...
(*  ignore (printf "Var %a\n" d_lval lval);*)
  try Depbase.is_immediate_type (Cilfacade.typeOfLval lval) || is_atomic lval
  with Not_found -> false

let get_flag (state: (string * Obj.t) list) : BS.Flag.t =
  snd (Obj.obj (List.assoc "depbase" state))
...
*)
  (* NB! Currently we care only about concrete indexes. Depbase (seeing only a int domain
...
(*     Messages.report (Printf.sprintf "%d + %d" (List.length accs_write) (List.length accs_read));   *)
  ...
  let add_concrete_access ctx fl loc ust (v, o, rv: varinfo * Offs.t * bool) =
    if (Depbase.is_global ctx.ask v) then begin
...
(*         else
  ...
  let init () =
    init ();
    arinc_analysis_activated := List.exists (fun x -> Json.string x="arinc" || Json.string x="arinc") (get_list "ana.activated[0]")
...
let _ =
  MCP.register_analysis ~dep:["depbase"] (module Spec : Spec)
