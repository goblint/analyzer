...
struct
  ...
  let evalbinop_base (a: Q.ask) emit (st: store) (op: binop) (t1:typ) (a1:value) (t2:typ) (a2:value) (t:typ) :value =
  ...
  let rec eval_rv (a: Q.ask) emit (gs:glob_fun) (st: store) (exp:exp): value =
  ...
  and eval_rv_base (a: Q.ask) emit (gs:glob_fun) (st: store) (exp:exp): value =
    ...
    let r =
      ...
      | Lval lv ->
        eval_rv_base_lval ~eval_lv a emit gs st exp lv
      (* Binary operators *)
      ...
      | BinOp ((Eq | Ne) as op, (CastE (t1, e1) as c1), (CastE (t2, e2) as c2), typ) when typeSig t1 = typeSig t2 ->
        ...
        let extra_is_safe =
          match evalbinop_base a emit st op t1 a1 t2 a2 typ with
  ...
  and eval_rv_base_lval ~eval_lv (a: Q.ask) emit (gs:glob_fun) (st: store) (exp: exp) (lv: lval): value =
    match lv with
    | (Var v, ofs) -> get a gs st (eval_lv a emit gs st (Var v, ofs)) (Some exp)
    ...
    | (Mem e, ofs) ->
      ...
      let lookup_with_offs addr =
        ...
        let v' = VD.eval_offset (Queries.to_value_domain_ask a) (fun x -> get a gs st x (Some exp)) v' (convert_offset a emit gs st ofs) (Some exp) None t in (* handle offset *)
        v'
      in
      AD.fold (fun a acc -> VD.join acc (lookup_with_offs a)) p (VD.bot ())

  and evalbinop (a: Q.ask) emit (gs:glob_fun) (st: store) (op: binop) ~(e1:exp) ?(t1:typ option) ~(e2:exp) ?(t2:typ option) (t:typ): value =
    evalbinop_mustbeequal a emit gs st op ~e1 ?t1 ~e2 ?t2 t

  (** Evaluate BinOp using MustBeEqual query as fallback. *)
  and evalbinop_mustbeequal (a: Q.ask) emit (gs:glob_fun) (st: store) (op: binop) ~(e1:exp) ?(t1:typ option) ~(e2:exp) ?(t2:typ option) (t:typ): value =
    ...
    let r = evalbinop_base a emit st op t1 a1 t2 a2 t in
  ...
  and eval_fv a emit (gs:glob_fun) st (exp:exp): AD.t =
    match exp with
    | Lval lval -> eval_lv a emit gs st lval
    | _ -> eval_tv a emit gs st exp
  (* Used also for thread creation: *)
  and eval_tv a emit (gs:glob_fun) st (exp:exp): AD.t =
  ...
  and convert_offset a emit (gs:glob_fun) (st: store) (ofs: offset) =
    let eval_rv = eval_rv_back_up in
    match ofs with
    | NoOffset -> `NoOffset
    ...
    | Index (exp, ofs) ->
      ...
      | Address add -> `Index (AD.to_int add, convert_offset a emit gs st ofs)
  ...
  and eval_lv (a: Q.ask) emit (gs:glob_fun) st (lval:lval): AD.t =
    let eval_rv = eval_rv_back_up in
    match lval with
    (* The simpler case with an explicit variable, e.g. for [x.field] we just
     * create the address { (x,field) } *)
    | Var x, ofs ->
      AD.singleton (Addr.of_mval (x, convert_offset a emit gs st ofs))
  ...
  let eval_rv (a: Q.ask) emit (gs:glob_fun) (st: store) (exp:exp): value =
    try
  ...
  let query ctx (type a) (q: a Q.t): a Q.result =
    ...
    | Q.EvalJumpBuf e ->
      begin match eval_rv_address (Analyses.ask_of_ctx ctx) ctx.emit ctx.global ctx.local e with
        | Address jmp_buf ->
    ...
    | Q.EvalMutexAttr e -> begin
        ...
        match eval_rv (Analyses.ask_of_ctx ctx) ctx.emit ctx.global ctx.local e with
        | MutexAttr a -> a
    ...
    | Q.EvalValue e ->
      eval_rv (Analyses.ask_of_ctx ctx) ctx.emit ctx.global ctx.local e
    | Q.BlobSize e -> begin
  ...
  struct
    ...
    let eval_rv_lval_refine a emit gs st exp lval = eval_rv a emit gs st (Lval lval)
  ...
  let branch ctx (exp:exp) (tv:bool) : store =
    ...
    let refine () =
      let res = invariant ctx (Analyses.ask_of_ctx ctx) ctx.emit ctx.global ctx.local exp tv in
      ...
      | s when Queries.ES.cardinal s = 1 ->
        ...
        invariant ctx (Analyses.ask_of_ctx ctx) ctx.emit ctx.global res e tv
      | _ -> res
    in
  ...
  let collect_funargs ask ?(warn=false) emit (gs:glob_fun) (st:store) (exps: exp list) =
    let do_exp e =
  ...
  let collect_invalidate ~deep ask ?(warn=false) emit (gs:glob_fun) (st:store) (exps: exp list) =
    if deep then
      collect_funargs ask ~warn emit gs st exps
    else (
  ...
  let invalidate ?(deep=true) ~ctx ask emit (gs:glob_fun) (st:store) (exps: exp list): store =
    ...
    let invalidate_exp exps =
      let args = collect_invalidate ~deep ~warn:true ask emit gs st exps in
  ...
  let assert_fn ctx e refine =
    ...
    if not refine then ctx.local else begin
      let newst = invariant ctx (Analyses.ask_of_ctx ctx) ctx.emit ctx.global ctx.local e true in
  ...
  let special_unknown_invalidate ctx ask gs st f args =
    ...
    let st' = invalidate ~deep:false ~ctx (Analyses.ask_of_ctx ctx) ctx.emit gs st shallow_addrs in
    invalidate ~deep:true ~ctx (Analyses.ask_of_ctx ctx) ctx.emit gs st' deep_addrs

  let check_invalid_mem_dealloc ctx special_fn ptr =
    match eval_rv_address (Analyses.ask_of_ctx ctx) ctx.emit ctx.global ctx.local ptr with
    | Address a ->
  ...
  let special ctx (lv:lval option) (f: varinfo) (args: exp list) =
    let invalidate_ret_lv st = match lv with
      | Some lv ->
        ...
        invalidate ~ctx (Analyses.ask_of_ctx ctx) ctx.emit ctx.global st [Cil.mkAddrOrStartOf lv]
      | None -> st
    in
    ...
    let memory_copying dst src =
      ...
      let src_typ = eval_lv (Analyses.ask_of_ctx ctx) ctx.emit gs st src_lval
                    |> AD.type_of in
    ...
    let eval_n = function
      ...
      | Some n ->
        begin match eval_rv (Analyses.ask_of_ctx ctx) ctx.emit gs st n with
          | Int i ->
            begin match ID.to_int i with
    ...
    let string_manipulation s1 s2 lv all op =
      ...
      | Some lv_val, Some f ->
        ...
        let lv_a = eval_lv (Analyses.ask_of_ctx ctx) ctx.emit gs st lv_val in
    ...
    | Strlen s, _ ->
      begin match lv with
        | Some lv_val ->
          let dest_a = eval_lv (Analyses.ask_of_ctx ctx) ctx.emit gs st lv_val in
    ...
    | MutexAttrSetType {attr = attr; typ = mtyp}, _ ->
      begin
        ...
        match eval_rv (Analyses.ask_of_ctx ctx) ctx.emit gs st mtyp with
        | Int x ->
          begin
      ...
      let st' =
        ...
        | Address ret_a ->
          begin match eval_rv (Analyses.ask_of_ctx ctx) ctx.emit gs st id with
            ...
            | _      -> invalidate ~ctx (Analyses.ask_of_ctx ctx) ctx.emit gs st [ret_var]
          end
        | _      -> invalidate ~ctx (Analyses.ask_of_ctx ctx) ctx.emit gs st [ret_var]
      in
    ...
    | Calloc { count = n; size }, _ ->
      begin match lv with
        | Some lv -> (* array length is set to one, as num*size is done when turning into `Calloc *)
          ...
          let ik = Cilfacade.ptrdiff_ikind () in
          let sizeval = eval_int (Analyses.ask_of_ctx ctx) ctx.emit gs st size in
          let countval = eval_int (Analyses.ask_of_ctx ctx) ctx.emit gs st n in
          if ID.to_int countval = Some Z.one then (
            set_many ~ctx (Analyses.ask_of_ctx ctx) gs st [
              ...
              (eval_lv (Analyses.ask_of_ctx ctx) ctx.emit gs st lv, (Cilfacade.typeOfLval lv), Address (add_null (AD.of_var heap_var)))
            ]
          )
          else (
            ...
            set_many ~ctx (Analyses.ask_of_ctx ctx) gs st [
              ...
              (eval_lv (Analyses.ask_of_ctx ctx) ctx.emit gs st lv, (Cilfacade.typeOfLval lv), Address (add_null (AD.of_mval (heap_var, `Index (IdxDom.of_int (Cilfacade.ptrdiff_ikind ()) BI.zero, `NoOffset)))))
            ]
          )
        | _ -> st
      end
    ...
    | Setjmp { env }, _ ->
      let ask = Analyses.ask_of_ctx ctx in
      let st' = match eval_rv ask ctx.emit gs st env with
        | Address jmp_buf ->
      ...
      begin match lv with
        | Some lv ->
          set ~ctx ask gs st' (eval_lv ask ctx.emit ctx.global st lv) (Cilfacade.typeOfLval lv) (Int (ID.of_int IInt BI.zero))
        | None -> st'
      end
    | Longjmp {env; value}, _ ->
      let ask = Analyses.ask_of_ctx ctx in
      let ensure_not_zero (rv:value) = match rv with
        ...
        | _ ->
          ...
          rv
      in
      let rv = ensure_not_zero @@ eval_rv ask ctx.emit ctx.global ctx.local value in
    ...
    | Rand, _ ->
      begin match lv with
        | Some x ->
          ...
          set ~ctx (Analyses.ask_of_ctx ctx) gs st (eval_lv (Analyses.ask_of_ctx ctx) ctx.emit ctx.global st x) (Cilfacade.typeOfLval x) result
        | None -> st
      end
    | _, _ ->
      let st =
  ...
  let combine_st ctx (local_st : store) (fun_st : store) (tainted_lvs : Q.LS.t) : store =
    ...
    Q.LS.fold (fun (v, o) st ->
        if CPA.mem v fun_st.cpa then
          ...
          let address = eval_lv ask ctx.emit ctx.global st lval in
    ...
    let e_d =
      ...
      let f st =
        ...
        struct
          ...
          let eval_rv a emit gs st e = eval_rv oa ctx.emit gs ost e
          let eval_rv_address a emit gs st e = eval_rv_address oa ctx.emit gs ost e
          let eval_lv a emit gs st lv = eval_lv oa ctx.emit gs ost lv
          let convert_offset a emit gs st o = convert_offset oa ctx.emit gs ost o
          ...
          let eval_rv_lval_refine a emit gs st exp lv =
            (* new, use different ctx for eval_lv (for Mem): *)
            eval_rv_base_lval ~eval_lv a ctx.emit gs st exp lv
        ...
        try
          Unassume.invariant ctx (Analyses.ask_of_ctx ctx) ctx.emit ctx.global ctx.local e true
