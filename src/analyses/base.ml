...
struct
      ...
      let lookup_with_offs addr =
        let v = (* abstract base value *)
          if cast_ok addr then
            get_addr ~man ~top:(VD.top_value t Unknown) st addr (Some exp)    (* downcasts are safe *)
          else
            VD.top () (* upcasts not! *)
        in
  ...
  *)
    ...
    | Q.ReachableFrom e -> begin
        ...
        | Int i ->
          begin match Cilfacade.typeOf e with
            | t when Cil.isPointerType t -> AD.of_int ~uk:Cast i (* integer used as pointer *)
            | _
  ...
  let set_savetop ~man ?lval_raw ?rval_raw st adr lval_t v : store =
    ...
    match v with
    | Top -> set ~man st adr lval_t (VD.top_value (AD.type_of adr) Unknown) ?lval_raw ?rval_raw
  ...
  let assign man (lval:lval) (rval:exp):store  =
    ...
    let not_local xs =
      let not_local x =
        match x with
        | Addr.Addr (x,_) -> is_global (Analyses.ask_of_man man) x
  ...
  let invalidate ~(must: bool) ?(deep=true) ~man (st:store) (exps: exp list) (uk:AddressDomain.unknownKind): store =
    ...
    let invalidate_addr (a: Addr.t) =
      ...
      let nv =  VD.invalidate_value (Queries.to_value_domain_ask (Analyses.ask_of_man man)) t v uk in
      (a, t, nv)
    in
    let invalids =
  ...
  let special_unknown_invalidate man f args =
    ...
    let st' = invalidate ~must:false ~deep:false ~man man.local shallow_addrs Invalidate in
    invalidate ~must:false ~deep:true ~man st' deep_addrs Invalidate
  ...
  let special man (lv:lval option) (f: varinfo) (args: exp list) =
    let invalidate_ret_lv st =
      Option.map_default (fun lv ->
        ...
        invalidate ~must:true ~deep:false ~man st [Cil.mkAddrOrStartOf lv] Invalidate
        ) st lv
    in
        ...
        begin match lv, op_addr with
          | Some lv_val, Some f ->
            ...
            if all && typeSig s1_typ = typeSig s2_typ && typeSig s2_typ = typeSig lv_typ then (* all types need to coincide *)
              ...
              set ~man st lv_a lv_typ (VD.top_value (unrollType lv_typ) String)
          | _ ->
            ...
            set ~man st s1_a s1_typ (VD.top_value (unrollType s1_typ) String)
        end
      ...
      Option.map_default (fun lv ->
          ...
          let value:value =
            ...
            if CilType.Typ.equal (AD.type_of a) charPtrType then
              ...
              | _ -> VD.top_value (unrollType dest_typ) String
          in
          set ~man st dest_a dest_typ value
        ) st lv
      ...
      let st' =
        ...
        | Address ret_a ->
          begin match eval_rv ~man st id with
            | Thread a when ValueDomain.Threads.is_top a -> invalidate ~must:true ~man st [ret_var] Invalidate
            | Thread a ->
            ...
            | _      -> invalidate ~must:true ~man st [ret_var] TypeMismatch
          end
        | _      -> invalidate ~must:true ~man st [ret_var] TypeMismatch
      in
