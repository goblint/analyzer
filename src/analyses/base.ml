...
struct
  ...
  let get_ptrs (vals: value list): address list =
    let f x acc = match x with
      | `Address adrs when AD.is_top adrs ->
        M.mywarn_each (M.LogEvent.may (M.EventType.Unknown ("Unknown address given as function argument"))); acc
      | `Address adrs when AD.to_var_may adrs = [] -> acc
      | `Address adrs ->
        let typ = AD.get_type adrs in
        if isFunctionType typ then acc else adrs :: acc
      | `Top -> M.mywarn_each (M.LogEvent.may (M.EventType.Unknown ("Unknown value type given as function argument"))); acc
      | _ -> acc
    in
  ...
  let rec reachable_from_value (ask: Q.ask) (gs:glob_fun) st (value: value) (t: typ) (description: string)  =
    ...
    | `Top ->
      ...
      if VD.is_immediate_type t then () else M.mywarn_each (M.LogEvent.may (M.EventType.Unknown (warning))); empty
    ...
    | `Address adrs when AD.is_top adrs ->
      ...
      M.mywarn_each (M.LogEvent.may (M.EventType.Unknown (warning))); AD.remove Addr.NullPtr adrs (* return known addresses still to be a bit more sane (but still unsound) *)
                ...
                begin
                  ...
                  | _ ->
                    if contains_vla t || contains_vla (get_type_addr a) then
                      begin
                        (* TODO: Is this ok? *)
                        M.mywarn (M.LogEvent.may (M.EventType.Unknown ("Casting involving a VLA is assumed to work")));
                        true
                      end
                    else
                      false
  ...
  let eval_funvar ctx fval: varinfo list =
    try
      ...
      if AD.mem Addr.UnknownPtr fp then begin
        M.mywarn_each (M.LogEvent.may (M.EventType.Unknown ("Function pointer " ^ sprint d_exp fval ^ " may contain unknown functions.")));
        dummyFunDec.svar :: AD.to_var_may fp
      end else
        AD.to_var_may fp
    with SetDomain.Unsupported _ ->
      M.mywarn_each (M.LogEvent.may (M.EventType.Unknown ("Unknown call to function " ^ sprint d_exp fval ^ ".")));
      [dummyFunDec.svar]
  ...
  let query ctx (type a) (q: a Q.t): a Q.result =
    ...
    | Q.EvalInt e -> begin
        ...
        | v      -> M.mywarn (M.LogEvent.may (M.EventType.Unknown ("Query function answered " ^ (VD.show v)))); Queries.Result.top q
      end
  ...
  * precise information about arrays. *)
  let set (a: Q.ask) ?(ctx=None) ?(effect=true) ?(change_array=true) ?lval_raw ?rval_raw ?t_override (gs:glob_fun) (st: store) (lval: AD.t) (lval_type: Cil.typ) (value: value) : store =
        ...
        | None ->
          if a.f (Q.IsHeapVar x) then
            ...
            with _ ->
              ...
              M.mywarn (M.LogEvent.may (M.EventType.Unknown ("Cil.typeOfLval failed Could not obtain the type of "^ sprint d_lval (Var x, cil_offset))));
              lval_type
      in
    ...
    | SetDomain.Unsupported x ->
      ...
      M.mywarn_each (M.LogEvent.may (M.EventType.Unknown ("Assignment to unknown address"))); st
 ...
 (**************************************************************************
  ...
  let invariant ctx a (gs:glob_fun) st exp tv =
    ...
    | None ->
      ...
      M.mywarn_each (M.LogEvent.may (M.EventType.Unknown ("Invariant failed: expression \"" ^ sprint d_plainexp exp ^ "\" not understood.")));
      st
    ...
    let inv_bin_int (a, b) ikind c op =
      let warn_and_top_on_zero x =
        if GU.opt_predicate (BI.equal BI.zero) (ID.to_int x) then
          (M.mywarn (M.LogEvent.may (M.EventType.Unknown ("Must Undefined Behavior: Second argument of div or mod is 0, continuing with top")));
          ID.top_of ikind)
        else
          x
      in
    ...
    | _ ->
      ...
      | Some exp ->
        let t_override = match fundec.svar.vtype with
          | TFun(TVoid _, _, _, _) -> M.mywarn (M.LogEvent.may (M.EventType.Unknown ("Returning a value from a void function"))); assert false
          | TFun(ret, _, _, _) -> ret
          | _ -> assert false
        in
  ...
  let invalidate ?ctx ask (gs:glob_fun) (st:store) (exps: exp list): store =
    ...
    if exps <> [] then M.mywarn_each (M.LogEvent.may (M.EventType.Unknown ("Invalidating expressions: " ^ sprint (d_list ", " d_plainexp) exps)));
  ...
  let forkfun (ctx:(D.t, G.t, C.t) Analyses.ctx) (lv: lval option) (f: varinfo) (args: exp list) : (lval option * varinfo * exp list) list =
    let create_thread lval arg v =
      ...
      with Not_found ->
        if LF.use_special f.vname then None (* we handle this function *)
          ...
          M.mywarn_each (M.LogEvent.may (M.EventType.Unknown ("Creating a thread from unknown function " ^ v.vname)));
          Some (lval, v, args)
        ) else (
          M.mywarn_each (M.LogEvent.may (M.EventType.Unknown ("Not creating a thread from " ^ v.vname ^ " because its type is " ^ sprint d_type v.vtype)));
          None
        )
    in
    ...
    | `Unknown _ when get_bool "sem.unknown_function.spawn" -> begin
        ...
        if addrs <> [] then M.mywarn_each (M.LogEvent.may (M.EventType.Unknown ("Spawning functions from unknown function: " ^ sprint (d_list ", " d_varinfo) addrs)));
  ...
  let assert_fn ctx e should_warn change =
    ...
    let warn ?annot msg = if should_warn then
        if get_bool "dbg.regression" then ( (* This only prints unexpected results (with the difference) as indicated by the comment behind the assert (same as used by the regression test script). *)
          ...
          if expected <> annot then (
            ...
            M.mywarn_each ~ctx:ctx.control_context (M.LogEvent.may (M.EventType.Unknown (msg ^ " Expected: " ^ (expected |? "SUCCESS") ^ " -> " ^ result)))
          )
        ) else
          M.mywarn_each ~ctx:ctx.control_context (M.LogEvent.may (M.EventType.Unknown msg))
    in
    ...
    | `Lifted true ->
      ...
      ctx.local
    | `Bot ->
      M.mywarn_each ~ctx:ctx.control_context (M.LogEvent.may (M.EventType.Unknown ("{red}Assertion \"" ^ expr ^ "\" produces a bottom. What does that mean? (currently uninitialized arrays' content is bottom)")));
      ctx.local
    | `Top ->
  ...
  let special ctx (lv:lval option) (f: varinfo) (args: exp list) =
    ...
    | _ -> begin
        let st =
          ...
          | None -> (
              (if not (CilType.Varinfo.equal f dummyFunDec.svar) && not (LF.use_special f.vname) then M.mywarn_each (M.LogEvent.may (M.EventType.Unknown ("Function definition missing for " ^ f.vname))));
              (if CilType.Varinfo.equal f dummyFunDec.svar then M.mywarn_each (M.LogEvent.may (M.EventType.Unknown ("Unknown function ptr called"))));
              let addrs =
                if get_bool "sem.unknown_function.invalidate.globals" then (
                  M.mywarn_each (M.LogEvent.may (M.EventType.Unknown ("INVALIDATING ALL GLOBALS!")));
