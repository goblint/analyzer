...
struct
  ...
  (* let invariant a gs st exp tv = *)
  (*   let fallback reason = *)
  (*     if M.tracing then M.tracel "inv" "Can't handle %a.\n%s\n" d_plainexp exp reason; *)
  (*     fst (invariant a gs st exp tv) *)
  (*   in *)
  (*   (* inverse values for binary operation a `op` b == c *) *)
  (*   let inv_bin_int (a, b) c = function *)
  (*     | PlusA  -> ID.meet a (ID.sub c b), ID.meet b (ID.sub c a) *)
  (*     | MinusA -> ID.meet a (ID.add c b), ID.meet b (ID.add c a) *)
  (*     | Mult   -> ID.meet a (ID.div c b), ID.meet b (ID.div c a) *)
  (*     | Div    -> ID.meet a (ID.mul c b), ID.meet b (ID.mul c a) *)
  (*     (* | Mod    -> ID.meet a (ID.mul c b), ID.meet b (ID.mul c a) *) *)
  (*     | Le -> *)
  (*       (match ID.minimal a, ID.maximal b with *)
  (*       | Some l1, Some u2 -> ID.meet a (ID.ending u2), ID.meet b (ID.starting l1) *)
  (*       | _ -> a, b) *)
  (*     | op -> *)
  (*       if M.tracing then M.tracel "inv" "Unhandled operator %s\n" (Deriving.Cil.show_binop op); *)
  (*       a, b *)
  (*   in *)
  (*   let inv_bin (a, b) c = function *)
  (*     | Eq | Ne as op -> *)
  (*       let x = ID.meet a b in *)
  (*       let both x = x, x in *)
  (*       (match op, ID.to_bool c with *)
  (*       | Eq, Some true *)
  (*       | Ne, Some false -> both x (* equal *) *)
  (*       | _ -> (* unequal *) *)
  ...
  (*   let null_val = function *)
  (*     | TPtr _ -> `Address AD.null_ptr *)
  (*     | _      -> `Int (ID.of_int 0L) *)
  (*   in *)
  (*   let rec inv_exp c = function *)
  (*     | BinOp (op, e1, e2, _) as e -> *)
  (*       if M.tracing then M.tracel "inv" "binop %a with abstract values %a and %a\n" d_plainexp e VD.pretty (eval e1) VD.pretty (eval e2); *)
  (*       (match eval e1, eval e2 with *)
  (*       | `Int a, `Int b -> *)
  (*         let a', b' = inv_bin (a, b) c op in *)
  ...
  (*         | _ -> `Int c *)
  (*       in *)
  (*       fst (set a gs st (eval_lv a gs st x) (VD.meet (eval (Lval x)) c') ~effect:false) *)
  (*     | Const c -> fst st *)
  (*     | e -> fallback (sprint d_plainexp e ^ " not implemeted") *)
  (*   in *)
  (*   inv_exp (ID.of_bool tv) exp, snd st *)
