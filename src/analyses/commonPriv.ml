...
struct
  ...
  let mutex_global: varinfo -> varinfo =
    let module VarinfoMap = (val RichVarinfo.Variables.map ~name:(fun x -> "MUTEX_GLOBAL_" ^ x.vname) (): RichVarinfo.Variables.VarinfoMap) (* explicit type to force call without ?size *) in
