...
type spec_modules = { spec : (module ArincSpec)
...
let register_analysis =
  let count = ref 0 in
  fun ?(dep=[]) (module S:ArincSpec) ->
    let s = { spec = (module S : ArincSpec)
...
struct
  ...
  let init () =
    ...
    (*iter (fun (x,y) -> Printf.printf "%s -> %a\n"  (flip assoc !analyses_table x) (List.print (fun f -> String.print f % flip assoc !analyses_table)) y) !dep_list_trans;
      ...
      Printf.printf "\n";*)
    iter (fun (_,{spec=(module S:ArincSpec); _}) -> S.init ()) !analyses_list

  let finalize () = iter (fun (_,{spec=(module S:ArincSpec); _}) -> S.finalize ()) !analyses_list
  ...
  let map_deadcode f xs =
    ...
    let one_el xs (n,(module S:ArincSpec),d) = try f xs (n,(module S:ArincSpec),d) :: xs with Deadcode -> dead:=true; (n,repr @@ S.D.bot ()) :: xs in
  ...
  let context x =
    ...
    map (fun (n,(module S:ArincSpec),d) ->
  ...
  let should_join x y =
    ...
    in
    let should_join ((_,(module S:ArincSpec),_),(_,x),(_,y)) = S.should_join (obj x) (obj y) in
  ...
  let otherstate v = map (fun (n,{spec=(module S:ArincSpec); _}) -> n, repr @@ S.otherstate v) !analyses_list
  let exitstate  v = map (fun (n,{spec=(module S:ArincSpec); _}) -> n, repr @@ S.exitstate  v) !analyses_list
  let startstate v = map (fun (n,{spec=(module S:ArincSpec); _}) -> n, repr @@ S.startstate v) !analyses_list
  ...
  let do_spawns ctx (xs:(varinfo * (int * Obj.t)) list) =
    let spawn_one v d =
      let join_vals (n,(module S:ArincSpec),d) =
  ...
  let do_assigns ctx assigns (xs:(int * Obj.t) list) =
    if List.is_empty assigns then xs (* nothing to do *)
      ...
      let spec_assign n d : Obj.t =
        ...
        let (module S:ArincSpec) = spec n in
  ...
  and query (ctx:(D.t, G.t, C.t) ctx) q =
    let sides  = ref [] in
    let f a (n,(module S:ArincSpec),d) =
  ...
  and part_access ctx (e:exp) (vo:varinfo option) (w: bool) =
    ...
    let sides  = ref [] in
    let f (po,lo) (n, (module S: ArincSpec), d) : part =
      let ctx' : (S.D.t, S.G.t, S.C.t) ctx =
        { local  = obj d
        ; node   = ctx.node
        ; control_context = ctx.control_context
  ...
  let arinc_edge (ctx:(D.t, G.t, C.t) ctx) e =
    let spawns = ref [] in
    let splits = ref [] in
    let sides  = ref [] in
    let f post_all (n,(module S:ArincSpec),d) =
  ...
  let sync (ctx:(D.t, G.t, C.t) ctx) =
    ...
    let splits = ref [] in
    let sides  = ref [] in
    let f (n,(module S:ArincSpec),d) (dl,cs) =
      let ctx' : (S.D.t, S.G.t, S.C.t) ctx =
        { local  = obj d
        ; node   = ctx.node
        ; control_context = ctx.control_context
