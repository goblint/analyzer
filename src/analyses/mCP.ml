...
struct
  ...
  let compatible (d : D.t) (a : P.t) (b : P.t) : bool =
    let zipped = List.fold_left2 (fun acc (n, a) (m, b) ->  assert (n = m); (n, (a, b))::acc) [] a b in
    let zipped_with_spec = spec_list zipped in
    let compatible (n, (module S: MCPSpec), (a, b)) =
      S.compatible (Obj.obj (List.assoc n d)) (Obj.obj a) (Obj.obj b)
    in
