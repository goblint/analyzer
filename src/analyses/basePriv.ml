...
(** Simplified [Analyses.ctx] because [Base] doesn't have [Analyses.ctx] to pass to us. *)
type ('d, 'g, 'c) ctx = {
  local: 'd;
  global: varinfo -> 'g;
...
sig
  ...
  val read_global: (BaseComponents (D).t, G.t, 'c) ctx -> varinfo -> VD.t
...
struct
  ...
  let read_global ctx x =
    ctx.global x

  let is_private (a: Q.ask) (v: varinfo): bool = false

  let write_global ctx ?(invariant=false) x v =
    let st = ctx.local in
    if invariant && not (is_private (ask_of_ctx ctx) x) then (
...
struct
  ...
  let read_global ctx x =
    match CPA.find x ctx.local.cpa with
...
struct
  ...
  let read_global ctx x =
    if is_unprotected (ask_of_ctx ctx) x then
      let get_mutex_global_x = get_mutex_global_x_with_mutex_inits ctx.global x in
      get_mutex_global_x |? VD.bot ()
    else
      CPA.find x ctx.local.cpa
  ...
  let lock ctx m =
    let st = ctx.local in
    let ask = ask_of_ctx ctx in
    let get_m = get_m_with_mutex_inits ask ctx.global m in
...
struct
  ...
  let read_global ctx x =
    ...
    if is_unprotected (ask_of_ctx ctx) x then (
      ...
      CPA.find x st.cpa
  let read_global ctx x =
    let v = read_global ctx x in
    if M.tracing then M.tracel "priv" "READ GLOBAL %a %B %a = %a\n" d_varinfo x (is_unprotected (ask_of_ctx ctx) x) CPA.pretty ctx.local.cpa VD.pretty v;
    v
  let write_global ctx ?(invariant=false) x v =
    let st = ctx.local in
    let cpa' =
...
struct
  ...
  let read_global ctx x =
    ...
    if P.mem x st.priv then
      CPA.find x st.cpa
    else if is_unprotected (ask_of_ctx ctx) x then
      G.unprotected (ctx.global x) (* CPA unnecessary because all values in GUnprot anyway *)
    else
      VD.join (CPA.find x st.cpa) (G.protected (ctx.global x))

  let write_global ctx ?(invariant=false) x v =
    let st = ctx.local in
    ctx.sideg x (if !GU.earlyglobs then G.create_init v else G.create_unprotected v); (* earlyglobs workaround for 13/60 *)
    if is_unprotected (ask_of_ctx ctx) x then
      st
    else
  ...
  let sync ctx reason =
    ...
    | `Join -> (* required for branched thread creation *)
      let st' =
        CPA.fold (fun x v (st: BaseComponents (D).t) ->
            if is_global ask x && is_unprotected ask x then (
              ctx.sideg x (G.create_unprotected v);
              {st with cpa = CPA.remove x st.cpa; priv = P.remove x st.priv}
            )
            else
              st
...
struct
  ...
  let read_global ctx x =
    ...
    GWeak.fold (fun s' tm acc ->
        if Lockset.disjoint s s' then
          ...
          acc
      ) (fst (ctx.global (mutex_global x))) (CPA.find x ctx.local.cpa)
  ...
  let write_global ctx ?(invariant=false) x v =
    ...
    if not (!GU.earlyglobs && is_precious_glob x) then
      ctx.sideg (mutex_global x) (GWeak.singleton s (ThreadMap.singleton t v), GSync.bot ());
    {st with cpa = cpa'}
  ...
  let lock ctx m =
    ...
    let cpa' = GSync.fold (fun s' cpa' acc ->
        if Lockset.disjoint s s' then
          ...
          acc
      ) (snd (ctx.global (mutex_addr_to_varinfo m))) st.cpa
    in
    {st with cpa = cpa'}
  ...
  let unlock ctx m =
    ...
    in
    ctx.sideg (mutex_addr_to_varinfo m) (GWeak.bot (), GSync.singleton s side_cpa);
    st
  ...
  let sync ctx reason =
    ...
    match reason with
    | `Return -> (* required for thread return *)
      begin match ThreadId.get_current (ask_of_ctx ctx) with
        | `Lifted x when CPA.mem x st.cpa ->
          let v = CPA.find x st.cpa in
          ctx.sideg (mutex_global x) ((GWeak.singleton (Lockset.empty ()) (ThreadMap.singleton x v), GSync.bot ()));
          {st with cpa = CPA.remove x st.cpa}
        | _ ->
          st
      end
...
struct
  ...
  let write_global ctx ?(invariant=false) x v =
    ...
    let cpa' = CPA.add x v st.cpa in
    if not (!GU.earlyglobs && is_precious_glob x) then
      ctx.sideg (mutex_global x) (GWeak.singleton s v, GSync.bot ());
    {st with cpa = cpa'}
  ...
  let sync ctx reason =
    ...
    | `Return -> (* required for thread return *)
      begin match ThreadId.get_current (ask_of_ctx ctx) with
        | `Lifted x when CPA.mem x st.cpa ->
          let v = CPA.find x st.cpa in
          ctx.sideg (mutex_global x) ((GWeak.singleton (Lockset.empty ()) v, GSync.bot ()));
          {st with cpa = CPA.remove x st.cpa}
        | _ ->
          st
      end
...
struct
  ...
  let read_global ctx x =
    ...
    GWeak.fold (fun s' v acc ->
        if Lockset.disjoint s s' then
          ...
          acc
      ) (G.weak (ctx.global (mutex_global x))) (CPA.find x ctx.local.cpa)
  ...
  let write_global ctx ?(invariant=false) x v =
    ...
    let cpa' = CPA.add x v st.cpa in
    if not (!GU.earlyglobs && is_precious_glob x) then
      ctx.sideg (mutex_global x) (G.create_weak (GWeak.singleton s v));
  ...
  let lock ctx m =
    ...
    let cpa' = GSync.fold (fun s' cpa' acc ->
        if Lockset.disjoint s s' then
          ...
          acc
      ) (G.sync (ctx.global (mutex_addr_to_varinfo m))) st.cpa
    in
    {st with cpa = cpa'}
  ...
  let read_global ctx x =
    ...
    let d_sync = L.fold (fun m bs acc ->
        if not (MustVars.mem x (V.find m vv)) then
          let syncs = G.sync (ctx.global (mutex_addr_to_varinfo m)) in
    ...
    in
    let weaks = G.weak (ctx.global (mutex_global x)) in
...
struct
  ...
  let write_global ctx ?(invariant=false) x v =
    ...
    if not (!GU.earlyglobs && is_precious_glob x) then (
      let v = distr_init ctx.global x v in
      ctx.sideg (mutex_global x) (G.create_weak (GWeak.singleton s (GWeakW.singleton s v)))
    );
  ...
  let sync ctx reason =
    ...
    | `Return -> (* required for thread return *)
      begin match ThreadId.get_current (ask_of_ctx ctx) with
        | `Lifted x when CPA.mem x st.cpa ->
          let v = CPA.find x st.cpa in
          ctx.sideg (mutex_global x) (G.create_weak (GWeak.singleton (Lockset.empty ()) (GWeakW.singleton (Lockset.empty ()) v)));
          {st with cpa = CPA.remove x st.cpa}
        | _ ->
          st
      end
...
struct
  ...
  let read_global ctx x =
    let v = Priv.read_global ctx x in
...
struct
  ...
  let read_global ctx x =
    ...
    let getg x =
      ...
      r
    in
    let ctx' = {ctx with global = getg} in
    let v = Priv.read_global ctx' x in
    if M.tracing then M.traceu "priv" "-> %a\n" VD.pretty v;
    v

  let write_global ctx ?invariant x v =
    ...
    let r = write_global ctx' ?invariant x v in
