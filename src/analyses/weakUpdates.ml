...
(** Set of local variables that need to be weakly updated, because multiple reachable copies of them may exist on the call stack. *)
module WeakUpdates =
struct
...
struct
  ...
  let query man (type a) (q: a Queries.t): a Queries.result =
    match q with
    | Queries.IsMultiple v ->
      WeakUpdates.mem v man.local ||
  ...
  let make_entry (man:(D.t, G.t, C.t, V.t) Analyses.man) fundec args: D.t =
    ...
    WeakUpdates.join man.local (WeakUpdates.of_list reachable_other_copies)
