...
sig
  ...
  val eval_rv: BaseDomainEvents.t -> Queries.ask -> (V.t -> G.t) -> D.t -> exp -> VD.t
  val eval_rv_address: BaseDomainEvents.t -> Queries.ask -> (V.t -> G.t) -> D.t -> exp -> VD.t
  val eval_lv: BaseDomainEvents.t -> Queries.ask -> (V.t -> G.t) -> D.t -> lval -> AD.t
  val convert_offset: BaseDomainEvents.t -> Queries.ask -> (V.t -> G.t) -> D.t -> offset -> ID.t Offset.t

  val get_var: BaseDomainEvents.t -> Queries.ask -> (V.t -> G.t) -> D.t -> varinfo -> VD.t
  val get: BaseDomainEvents.t -> Queries.ask -> (V.t -> G.t) -> D.t -> AD.t -> exp option -> VD.t
  val set: Queries.ask -> ctx:(D.t, G.t, D.t, V.t) Analyses.ctx -> (V.t -> G.t) -> D.t -> AD.t -> typ -> ?lval_raw:lval -> VD.t -> D.t

  val refine_entire_var: bool
  val map_oldval: VD.t -> typ -> VD.t
  val eval_rv_lval_refine: BaseDomainEvents.t  -> Queries.ask -> (V.t -> G.t) -> D.t -> exp -> lval -> VD.t
...
struct
  ...
  let refine_lv_fallback ctx a gs st lval value tv =
    let emit = Events.to_base_domain_events ctx.Analyses.emit in
    if M.tracing then M.tracec "invariant" "Restricting %a with %a\n" d_lval lval VD.pretty value;
    let addr = eval_lv emit a gs st lval in
    if (AD.is_top addr) then st
    else
      let old_val = get emit a gs st addr None in (* None is ok here, we could try to get more precise, but this is ok (reading at unknown position in array) *)
      ...
      let value = get (Events.to_base_domain_events ctx.emit) a gs state_with_excluded addr None in
  ...
  let refine_lv ctx a gs st c x c' pretty exp =
    let set' lval v st = set a gs st (eval_lv (Events.to_base_domain_events ctx.emit) a gs st lval) (Cilfacade.typeOfLval lval) ~lval_raw:lval v ~ctx in
    match x with
    | Var var, o when refine_entire_var ->
      ...
      let old_val = get_var (Events.to_base_domain_events ctx.emit) a gs st var in
      let old_val = map_oldval old_val var.vtype in
      let offs = convert_offset (Events.to_base_domain_events ctx.emit) a gs st o in
    ...
    | Mem _, _ ->
      ...
      let old_val = eval_rv_lval_refine (Events.to_base_domain_events ctx.emit) a gs st exp x in
  ...
  let invariant_fallback ctx a (gs:V.t -> G.t) st exp tv =
    ...
    let rec helper (op: binop) (lval: lval) (value: VD.t) (tv: bool): (lval * VD.t) option =
      ...
      | Eq, x, value, false -> begin
          ...
          | Address n -> begin
              ...
              match eval_rv_address (Events.to_base_domain_events ctx.Analyses.emit) a gs st (Lval x) with
    ...
    let rec derived_invariant exp tv =
      ...
      | BinOp(op, Lval x, rval, typ) -> helper op x (VD.cast (Cilfacade.typeOfLval x) (eval_rv (Events.to_base_domain_events ctx.emit) a gs st rval)) tv
      ...
      | BinOp(op, CastE (TInt (ik, _) as t1, Lval x), rval, typ) ->
        (match eval_rv (Events.to_base_domain_events ctx.emit) a gs st (Lval x) with
         | Int v ->
  ...
  let invariant ctx a gs st exp tv: D.t =
    ...
    in
    let eval e st = eval_rv (Events.to_base_domain_events ctx.emit) a gs st e in
