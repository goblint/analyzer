      ...
      let report_race offset acc_list =
        ...
        let handle_race def_warn = begin
          if (List.mem gl.vname  (get_string_list "ana.osek.safe_vars")) then begin
            ...
            if (get_bool "allglobs") then
              msg_group_race_old (safe_str "safe variable") warnings
            else
            ...
            if safe_funs = [] then begin
              ...
              msg_group_race_old warn warnings
            end else begin
              filtered := !filtered +1;
              ...
              | Guarded locks ->
                ...
                if (get_bool "allglobs") then
                  msg_group_race_old (safe_str "common mutex after filtering") warnings
                else
                  ignore (printf "Found correlation: %s is guarded by lockset %s after filtering\n" var_str lock_str)
              | Priority pry ->
                if (get_bool "allglobs") then
                  msg_group_race_old (safe_str "same priority after filtering") warnings
                else
                  ignore (printf "Found correlation: %s is guarded by priority %s after filtering\n" var_str (string_of_int pry))
              | Defence (defpry,offpry) ->
                if (get_bool "allglobs") then
                  msg_group_race_old (safe_str "defensive priority exceeds offensive priority after filtering") warnings
                else
              ...
              | Flag (flagvar) ->
                if (get_bool "allglobs") then
                  msg_group_race_old (safe_str ("variable "^flagvar ^" used to prevent concurrent accesses after filtering") ) warnings
                else
                  ignore (printf "Found correlation: %s is guarded by flag %s after filtering\n" var_str flagvar)
              | ReadOnly ->
                if (get_bool "allglobs") then
                  msg_group_race_old (safe_str "only read after filtering") warnings
              | ThreadLocal ->
                if (get_bool "allglobs") then
                  msg_group_race_old (safe_str "thread local after filtering") warnings
              | BadFlag -> begin
                  race_free := false;
              ...
              | GoodFlag -> begin
                  if (get_bool "allglobs") then begin
                    msg_group_race_old (safe_str "is a flag after filtering") warnings
                  end else  begin
        ...
        let res = match is_race acc_list with
          ...
          | Guarded locks ->
            let lock_str = Mutex.Lockset.show locks in
            if (get_bool "allglobs") then
              msg_group_race_old (safe_str "common mutex") warnings
            else
              ignore (printf "Found correlation: %s is guarded by lockset %s\n" var_str lock_str)
          | Priority pry ->
            if (get_bool "allglobs") then
              msg_group_race_old (safe_str "same priority") warnings
            else
              ignore (printf "Found correlation: %s is guarded by priority %s\n" var_str (string_of_int pry))
          | Defence (defpry,offpry) ->
            if (get_bool "allglobs") then
              msg_group_race_old (safe_str "defensive priority exceeds offensive priority") warnings
            else
              ignore (printf "Found correlation: %s is guarded by defensive priority %s against offensive priority %s\n" var_str (string_of_int defpry) (string_of_int offpry))
          | Flag (flagvar) ->
            if (get_bool "allglobs") then
              msg_group_race_old (safe_str ("variable "^flagvar ^" used to prevent concurrent accesses") ) warnings
            else
              ignore (printf "Found correlation: %s is guarded by flag %s\n" var_str flagvar)
          | ReadOnly ->
            if (get_bool "allglobs") then
              msg_group_race_old (safe_str "only read") warnings
          | ThreadLocal ->
            if (get_bool "allglobs") then
              msg_group_race_old (safe_str "thread local") warnings
          | BadFlag -> begin
              race_free := false;
              let warn = "Writerace at 'flag' " ^ var_str in
              msg_group_race_old warn warnings
          ...
          | GoodFlag -> begin
              if (get_bool "allglobs") then begin
                msg_group_race_old (safe_str "is a flag") warnings
              end else  begin
