...
(**
   ...
   The generated copy-ready file combines the extracted identity-monad RHS
   evaluator with this OCaml-specific mutable representation. Ordinary owner
   phases reset at fresh episode boundaries and survive dirty reruns.  A side
   cell resets after the minimum-ranked variable read before that effect has
   published a changed, completed episode; read-free effects are stored raw.
      ...
      let rec solve_episode owner =
        ...
        let rec evaluate_owner () =
          ...
          let direct =
            ...
            | Some body ->
              Extracted.monadic_eval_rhs
                (get frame) (side frame) (spawn frame) body
            | None -> bottom
          in
...
let _ =
  Selector.add_solver ("qsi_w",    (module PostSolver.AddPost  (QSolverW)));
  Selector.add_solver ("qsi_wnw",  (module PostSolver.AddPost  (QSolverWNW)));
  Selector.add_solver ("qsi_cwnw", (module PostSolver.AddPost  (QSolverCWNW)))
