(* GENERATED FILE. DO NOT EDIT.
   ...
   Variant: ReluctantSimple
   Configuration: first-discovery Reluctant Simple; aggregate-covered side joins
   Adapter: extraction/reluctant/goblint_qsolver_reluctant_simple.ml.in
   Regenerate with: make goblint-reluctant-solvers *)
(**
   Goblint callback adapter for the Reluctant Simple QSolver.

   This is a mutable implementation of the algorithm in
   [reluctant_simple/QSOLVER_RELUCTANT_SIMPLE.md]. The control operations at
   [@@CONTROL@@] are
...
module Make
  ...
  struct
    ...
    let trace_algorithm = "ReluctantSimple"
    ...
    type rank_info = {
      ...
      mutable side_cells : side_row option;
    }
    ...
    let[@inline always] set_value state frame direct =
      ...
      let () =
        if not (lattice_equal candidate old) then
          if owner_info.controlled then begin
            let next_phase, next =
    ...
    let solve xs roots _old_data =
      ...
      let rec solve_episode owner =
        ...
        let owner_phase =
          match owner_info.normal_phase with
          | Some phase -> phase
      ...
      and write_side_contribution frame target input =
        ...
        let cells =
          match target_info.side_cells with
          | Some cells -> cells
          | None ->
            let cells = VH.create 4 in
            target_info.side_cells <- Some cells;
            cells
        in
        let cell =
        ...
        let next_phase, next_contribution =
          reluctant_side_update source target cell.phase
