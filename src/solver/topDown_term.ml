    ...
    let solve st vs =
      ...
      and solve x phase =
        ...
        if not (HM.mem called x || HM.mem stable x) then (
          ...
          HM.remove called x;
          let tmp = if wpx then match phase with Widen -> S.Dom.widen old (S.Dom.join old tmp) | Narrow -> S.Dom.narrow old tmp else tmp in (* TODO: remove join *)
          if not (S.Dom.equal old tmp) then (
            (* if tracing then if is_side x then trace "sol2" "solve side: old = %a, tmp = %a, widen = %a" S.Dom.pretty old S.Dom.pretty tmp S.Dom.pretty (S.Dom.widen old (S.Dom.join old tmp)); *) (* TODO: remove join *)
      ...
      and side y d =
        ...
        if not (S.Dom.leq d old) then (
          HM.replace rho' y (S.Dom.widen old (S.Dom.join old d)); (* TODO: remove join *)
          HM.remove stable y;
          init y;
          solve y Widen;
        )
