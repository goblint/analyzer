(* GENERATED FILE. DO NOT EDIT.
   ...
   Variant: Restarting
   Configuration: first-discovery instance; proved side-reset and dead-side switches enabled
   Adapter: extraction/goblint_qsolver_restarting.ml.in
   Regenerate with: make goblint-solvers *)
(**
   Goblint callback adapter for the Restarting QSolver.

   This is a mutable implementation of the algorithm in
   [restarting/QSOLVER_RESTARTING.md]. The control operations at [@@CONTROL@@]
...
module Make
  ...
  struct
    ...
    let trace_algorithm = "Restarting"
    ...
    let[@inline always] create_state () =
      {
        ...
        reset_at_close = None;
        last_targets = None;
        read_dependencies = VH.create 32;
        queue = create_queue ();
      }
    ...
    let solve xs roots _old_data =
      ...
      let rec solve_episode owner =
        ...
        let start_evaluation _ =
          begin match VH.find_option state.read_dependencies owner with
            | Some dependencies -> VH.clear dependencies
            | None -> ()
          end;
      ...
      and get frame target =
        ...
        VH.replace
          (get_or_create state.read_dependencies owner 4)
          target ();
      ...
      and write_side_contribution frame target input =
        ...
        let source_dependencies =
          VH.find_option state.read_dependencies source
        in
        let scope, scope_rank =
          match source_dependencies with
          | Some source_dependencies ->
            VH.fold
              (fun dependency () (minimum, minimum_rank) ->
                 let dependency_rank = rank state.queue dependency in
                 if dependency_rank < minimum_rank
                 then dependency, dependency_rank
                 else minimum, minimum_rank)
              source_dependencies
              (source, source_rank)
          | None -> source, source_rank
        in
