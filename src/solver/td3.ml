    ...
    let solve st vs marshal =
        ...
        if not (HM.mem called x || HM.mem stable x) then (
          ...
          let wpd = (* d after widen/narrow (if wp) *)
            if not wp then eqd
              ...
              match phase with
              | Widen -> S.Dom.widen old (S.Dom.join old eqd) (* TODO: remove join *)
      ...
      and side ?x y d = (* side from x to y; only to variables y w/o rhs; x only used for trace *)
        ...
        let widen a b =
          ...
          let r = S.Dom.widen a (S.Dom.join a b) in (* TODO: remove join *)
        ...
        if not (S.Dom.leq tmp old) then (
          ...
          (* HM.replace rho y ((if HM.mem wpoint y then S.Dom.widen old else identity) (S.Dom.join old d)); *) (* TODO: remove join *)
