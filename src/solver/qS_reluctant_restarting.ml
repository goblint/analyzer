(* GENERATED FILE. DO NOT EDIT.
   ...
   Variant: ReluctantRestarting
   Configuration: first-discovery Reluctant Restarting; both feature switches enabled
   Adapter: extraction/reluctant/goblint_qsolver_reluctant_restarting.ml.in
   Regenerate with: make goblint-reluctant-solvers *)
(**
   Goblint callback adapter for the Reluctant Restarting QSolver.

   This is a mutable implementation of the algorithm in
   [reluctant_restarting/QSOLVER_RELUCTANT_RESTARTING.md].
   The control operations at [@@CONTROL@@]
   are supplied either by Rocq extraction or by the explicitly inlined
   benchmark variant.
...
module Control = struct
  ...
  type ('v, 'd, 'monad) transfer =
    ('v -> 'monad) -> ('v -> 'd -> 'monad) -> ('v -> 'monad) -> 'monad

  type ('v, 'd, 'monad) system = 'v -> ('v, 'd, 'monad) transfer option
  ...
  (** val drain_with_fuel :
      (__ -> __ -> 'a2) -> (__ -> __ -> 'a2 -> (__ -> 'a2) -> 'a2) -> int ->
      (unit -> 'a2) -> ('a1 -> 'a2) -> 'a2 **)

  let rec drain_with_fuel ret0 bind0 fuel pop_owner solve_queued_owner =
    Obj.magic bind0 __ __ (pop_owner ()) (fun picked ->
        match picked with
        | Some owner ->
          ((fun fO fS n -> if n=0 then fO () else fS (n-1))
             (fun _ -> Obj.magic ret0 __ SeedsOutOfFuel)
             (fun remaining ->
                bind0 __ __ (solve_queued_owner owner) (fun _ ->
                    drain_with_fuel ret0 bind0 remaining pop_owner solve_queued_owner))
             fuel)
        | None -> Obj.magic ret0 __ (SeedsSolved fuel))

  (** val solve_seeds_with_fuel :
      (__ -> __ -> 'a2) -> (__ -> __ -> 'a2 -> (__ -> 'a2) -> 'a2) -> int ->
      ('a1 -> 'a2) -> (unit -> 'a2) -> ('a1 -> 'a2) -> 'a1 list -> 'a2 **)

  let rec solve_seeds_with_fuel ret0 bind0 fuel is_started pop_owner solve_owner = function
    | [] -> Obj.magic ret0 __ (SeedsSolved fuel)
    | root :: remaining_roots ->
      Obj.magic bind0 __ __ (is_started root) (fun started ->
          if started
          then solve_seeds_with_fuel ret0 bind0 fuel is_started pop_owner
              solve_owner remaining_roots
          else ((fun fO fS n -> if n=0 then fO () else fS (n-1))
                  (fun _ -> Obj.magic ret0 __ SeedsOutOfFuel)
                  (fun remaining_fuel ->
                     bind0 __ __ (solve_owner root) (fun _ ->
                         Obj.magic bind0 __ __
                           (drain_with_fuel ret0 bind0 remaining_fuel pop_owner
                              solve_owner)
                           (fun drained ->
                              match drained with
                              | SeedsSolved after_drain ->
                                solve_seeds_with_fuel ret0 bind0 after_drain is_started
                                  pop_owner solve_owner remaining_roots
                              | SeedsOutOfFuel -> Obj.magic ret0 __ SeedsOutOfFuel)))
                  fuel))
  ...
  type ('v, 'd) callback = ('v, 'd, __ m) system

  (** val eval_rhs :
      ('a1 -> 'a2 m) -> ('a1 -> 'a2 -> unit m) -> ('a1 -> unit m) -> ('a1, 'a2,
      __ m) transfer -> 'a2 m **)

  let[@inline always] eval_rhs get side spawn body =
    Obj.magic body get side spawn
  ...
  let[@inline always] solve_one bottom start_evaluation get side spawn set_value drain_owner system0 owner =
    bind (start_evaluation owner) (fun _ ->
        bind
          (match system0 owner with
         ...
         bind (pop_owner ()) (fun picked ->
             ...
             | Some owner ->
               bind (solve_owner owner) (fun _ ->
                   solve_loop_fuel remaining pop_owner solve_owner)
             | None -> ret true))
      fuel
  ...
  (** val drain_with_fuel0 :
      int -> (unit -> 'a1 option m) -> ('a1 -> unit m) -> seeded_result m **)

  let drain_with_fuel0 fuel pop_owner solve_owner =
    drain_with_fuel (Obj.magic (fun _ -> ret)) (Obj.magic (fun _ _ -> bind))
      fuel (Obj.magic pop_owner) (Obj.magic solve_owner)

  (** val solve_seeds_with_fuel0 :
      int -> ('a1 -> bool m) -> (unit -> 'a1 option m) -> ('a1 -> unit m) ->
      'a1 list -> seeded_result m **)

  let[@inline always] solve_seeds_with_fuel fuel is_started pop_owner solve_owner roots =
    solve_seeds_with_fuel (Obj.magic (fun _ -> ret))
      (Obj.magic (fun _ _ -> bind)) fuel (Obj.magic is_started)
      (Obj.magic pop_owner) (Obj.magic solve_owner) roots
end
...
module Features = struct
  (** The two compile-time policies exposed by the proved Restarting solver.
      The generated Goblint examples enable both. *)
...
module Make
  ...
  struct
    ...
    let trace_algorithm = "ReluctantRestarting"
    let trace_control = "extracted"
    ...
    type episode = {
      ...
      (** Minimum-ranked owner or dependency read in this evaluation. *)
    ...
    let[@inline always] reluctant_side_update
        _source target phase old input aggregate =
      let next_phase, next = U.update true target phase old input in
      if S.Dom.leq input old then next_phase, next
      else if S.Dom.leq input aggregate then
        next_phase, S.Dom.join old input
      else next_phase, next
    ...
    let solve xs roots _old_data =
        ...
        let start_evaluation _ =
          ...
          if Features.dead_side_elimination then begin
            frame.previous_targets <-
              begin match state.last_targets with
      ...
      and write_side_contribution frame target input =
        ...
        let next_phase, next_contribution =
          reluctant_side_update source target phase
            old_contribution input old_aggregate
        in
