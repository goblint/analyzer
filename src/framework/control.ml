...
module AnalyzeNeedCFG (Cfg:CfgBidir) =
struct
  
  (** The specification. *)
  module EQSys = Need.System (Cfg)
  ...
  module RT = 
  struct
    include Printable.Prod3 (EQSys.C) (EQSys.D) (Basetype.CilFundec)
    let isSimple _ = false
    let short w (es,x,f:t) = 
      match es with 
        | `F (cf, cb) -> "F-"^EQSys.B.call_descr f cf
        | `B (cf, cb) -> "B-"^EQSys.B.call_descr f cf
        | `C (n,f,cf,cb) -> "C-"^EQSys.B.call_descr f cf
    ...
    let toXML (es,x,_ as st:t) = 
      ...
      let ctx = try_replace_text "Context" (flatten_single (EQSys.C.toXML es)) in
      let res = Element("Node",["text","Value"], [EQSys.D.toXML x]) in
        Element ("Node",["text",esc (short 80 st)],[ctx;res])            
    let pretty () (_,x,_) = EQSys.D.pretty () x
  end
  
  (** Set of triples [RT] *)
  module LT = SetDomain.HeadlessSet (RT)
  (** Analysis result structure---a hashtable from porgram points to [LT] *)
  module Result = Analyses.Result (LT) (struct let result_name = "need" end)
  ...
  let make_global_xml g =
    let one_glob k v = 
      ...
      let varvalue = Xml.Element ("td",[],[EQSys.G.toXML v]) in
  ...
  let do_extern_inits ctx (file : Cil.file) : EQSys.B.D.t =
    ...
    let set_bad v st =
      EQSys.B.assign {ctx with local2 = st} (var v) MyCFG.unknown_exp 
    in
    let add_externs s = function
      | GVarDecl (v,_) when not (VS.mem v vars || isFunctionType v.vtype) -> set_bad v s
      | _ -> s
    in    
    Cil.foldGlobals file add_externs (EQSys.B.startstate ())

  (** analyze cil's global-inits function to get a starting state *)
  let do_global_inits (file: Cil.file) : EQSys.B.D.t * Cil.fundec list = 
    let ctx = 
      { ask2     = (fun _ -> Queries.Result.top ())
      ; local2   = EQSys.B.D.top ()
      ; global2  = (fun _ -> EQSys.B.G.bot ())
      ; presub2  = []
      ; postsub2 = []
    ...
    let funs = ref [] in
    let transfer_func (st : EQSys.B.D.t) (edge, loc) : EQSys.B.D.t = 
      try
        if M.tracing then M.trace "con" "Initializer %a\n" d_loc loc;
        Tracing.current_loc := loc;
        match edge with
          | MyCFG.Entry func        -> EQSys.B.body {ctx with local2 = st} func
          | MyCFG.Assign (lval,exp) -> 
              ...
              EQSys.B.assign {ctx with local2 = st} lval exp
          | _                       -> raise (Failure "This iz impossible!") 
      with Failure x -> M.warn x; st
    in
    let with_externs = do_extern_inits ctx file in
    let result : EQSys.B.D.t = List.fold_left transfer_func with_externs edges in
      result, !funs
  
  let print_globals glob = 
    let out = M.get_out EQSys.B.name !GU.out in
    let print_one v st =
      ignore (Pretty.fprintf out "%a -> %a\n" EQSys.GVar.pretty_trace v EQSys.B.G.pretty st)
    in
  ...
  let analyze (file: Cil.file) (startfuns, exitfuns, otherfuns: Analyses.fundecs) = 
    ...
    let enter_with st fd =
      ...
      let ents = EQSys.B.enter ctx None fd.Cil.svar args in
    ...
    let exitvars = List.map (enter_with (EQSys.B.exitstate ())) exitfuns in
    let othervars = List.map (enter_with (EQSys.B.otherstate ())) otherfuns in
    ...
    let do_analyze () = 
      ...
      (if (get_bool "dbg.debug") && EQSys.D.is_bot main_sol then
...
let analyze (file: Cil.file) fs = 
  ...
  if get_bool "exp.need" then
    let module A = AnalyzeNeedCFG (CFG) in
    A.analyze file fs 
  else
