...
let spec_module: (module Spec) Lazy.t = lazy (
  ...
  let termination = List.mem "termination" (get_string_list "ana.activated") in (* check if loop termination analysis is enabled*)
  ...
  let module S1 = (val
                    ...
                    |> lift termination (module RecursionTermLifter) (* Always activate the recursion termination analysis, when the loop termination analysis is activated*)
                  ) in
