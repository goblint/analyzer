    ...
    let do_analyze_using_solver () =
      ...
      let evaluate_single (_, location, _, identifier) =
        ...
        | Some (function_declaration, statement) ->
            let translation =
              match Hashtbl.find_option variables identifier with
            ...
            | `Top -> "Top"
            | _ -> raise Exit)
            (*analyzer#evaluate function_declaration statement expression
              |> List.map (List.fold_left (fun accumulator result -> accumulator ^ "\n" ^ result) "")
