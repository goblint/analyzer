...
struct
  ...
  let incr k =
    H.modify_def 1 k (fun v ->
        if v >= !limit then failwith ("LimitLifter: Reached limit ("^string_of_int !limit^") for node "^GobPretty.sprint Node.pretty_plain_short (Option.get !MyCFG.current_node));
        v+1
      ) h;
  module D = struct
    include S.D
