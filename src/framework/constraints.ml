...
struct
  ...
  let common_ctx var edge prev_node pval (getl:lv -> ld) sidel getg sideg : (D.t, G.t, S.C.t) ctx * D.t list ref * (lval option * varinfo * exp list * D.t) list ref =
    ...
    and spawn lval f args =
      ...
      List.iter (fun d ->
          ...
          | exception Not_found ->
            (* unknown function *)
            M.warn_each @@ Unknown ("Created a thread from unknown function " ^ f.vname)
  ...
  let tf_proc var edge prev_node lv e args getl sidel getg sideg d =
    ...
    let one_function f =
      ...
      | fd when LibraryFunctions.use_special f.vname ->
        M.warn_each @@ M.Unknown ("Using special for defined function " ^ f.vname);
  ...
  let tf (v,c) (e,u) getl sidel getg sideg =
    ...
    let d       = try tf (v,c) (e,u) getl sidel getg sideg
      with M.Bailure s -> Messages.warn_each @@ Messages.Unknown s; (getl (u,c))  in
