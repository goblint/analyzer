...
struct
    ...
    let one_function f =
      ...
      if has_dec then (
        if LibraryFunctions.use_special f.vname then (
          M.warn_each (M.Warning.may (M.WarnType.Unknown (("Using special for defined function " ^ f.vname))));
  ...
  let tf (v,c) (e,u) getl sidel getg sideg =
    ...
    let d       = try tf (v,c) (e,u) getl sidel getg sideg
      with M.Bailure s -> Messages.warn_each (Messages.Warning.may (Messages.WarnType.Unknown (s))); (getl (u,c))  in
