...
let analyze change_info =
  ...
  let cfgF, cfgB, startnode, taskinfo = Arinc_cfg.get_cfg () in
  let module CFG = struct let prev = cfgB let next = cfgF let startnode = startnode let taskinfo = taskinfo end in
