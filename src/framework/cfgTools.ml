...
let computeSCCs x = Stats2.time "computeSCCs" (computeSCCs x)
      ...
      | GFun (fd, fd_loc) ->
        ...
        let handle stmt =
          ...
          | ComputedGoto _ ->
            failwith "MyCFG.createCFG: unsupported stmt"
        in
        Stats2.time "handle" (List.iter handle) fd.sallstmts;
        ...
        in
        Stats2.time "iter_connect" iter_connect ();
...
let createCFG = Stats2.time "createCFG" createCFG
...
let getCFG (file: file) : cfg * cfg =
  ...
  let cfgF, cfgB =
    if get_bool "exp.mincfg" then
      Stats2.time "minimizing the cfg" minimizeCFG (cfgF, cfgB)
    else
      (cfgF, cfgB)
  in
