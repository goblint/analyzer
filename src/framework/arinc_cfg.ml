...
(* id * (priority * period option * capacity option) * cfg *)
type arinc_task_cfg = int * (int * int option * int option) * (task_node * (task_edge list * task_node) list) list [@@deriving yojson]
...
sig
  ...
  val taskinfo: (int * int option * int option) list
end
