...
let our_arinc_cfg:arinc_cfg*arinc_cfg =
  ...
  for i = 0 to 6 do
    ...
    mkEdge (PC ([2; i])) (0, SetEvent 0) (PC [3; i]);
    mkEdge (PC ([3; i])) (0, Computation 20) (PC [4; i]);
    mkEdge (PC ([4; i])) (0, ResumeTask 1) (PC [5; i]);
    mkEdge (PC ([5; i])) (0, WaitEvent 1) (PC [6; i]);
    mkEdge (PC ([6; i])) (0, ResetEvent 1) (PC [7; i]);
    mkEdge (PC ([7; i])) (0, Computation 42) (PC [8; i]);
    mkEdge (PC ([8; i])) (0, SuspendTask 1) (PC [9; i]);
    mkEdge (PC ([9; i])) (0, PeriodicWait) (PC [10; i]);
    mkEdge (PC ([10; i])) (0, NOP) (PC [3; i]);
  done;
  for i = 0 to 11 do
