...
module HashconsLifter (S:ArincSpec)
  : ArincSpecHC with module D = Lattice.HConsed (S.D)
          and module G = S.G
...
module HashconsContextLifter (S:ArincSpec)
  : ArincSpec with module D = S.D
          and module G = S.G
...
module NoHashconsLifter (S: ArincSpec) = struct
  module C = Printable.HC (S.C)
  include (S : ArincSpec with module C := C)
end

(* see option ana.opt.equal *)
module OptEqual (S: ArincSpec) = struct
  ...
  include (S : ArincSpec with module D := D and module G := G and module C := C)
end

(** If dbg.slice.on, stops entering functions after dbg.slice.n levels. *)
module LevelSliceLifter (S:ArincSpec)
  : ArincSpec with module D = Lattice.Prod (S.D) (Lattice.Reverse (IntDomain.Lifted))
          and module G = S.G
          and module C = S.C
=
struct
...
module LimitLifter (S:ArincSpec) =
struct
...
module WidenContextLifterSide (S:ArincSpec)
=
struct
...
module DeadCodeLifter (S:ArincSpec)
  : ArincSpec with module D = Dom (S.D)
          and module G = S.G
          and module C = S.C
=
struct
  ...
  let lift_fun ctx f g h b =
    ...
    with Deadcode ->
      begin
        ...
        | PCCombined [a;b] -> "[" ^ string_of_int(a) ^"," ^ string_of_int(b) ^ "]"
        | _ -> "" in
...
struct
  ...
  let system ((v,c): LVar.t) =
    ...
    (* Printf.printf "\tControl-Flow predecessors: %i\n" (List.length (Cfg.prev v));
      List.iter (fun (e,x) -> match x with PCCombined [a; b] -> Printf.printf "\t\t[%i,%i]\n" a b) (Cfg.prev v); *)
...
module GlobSolverFromEqSolver (Sol:GenericEqBoxSolver)
  ...
  = functor (S:GlobConstrSys) ->
    ...
    struct
      ...
      module EqSys = Generic_arinc.NormalSysConverter (IneqSys)
...
module PathSensitive2 (Spec:ArincSpec)
  : ArincSpec
