...
module FromSpec (S:Spec) (Cfg:CfgBackward)
  : sig
    include IncrGlobConstrSys with module LVar = VarF (S.C)
                           and module GVar = Basetype.Variables
                           and module D = S.D
                           and module G = S.G
    val tf : MyCFG.node * S.C.t  -> (Cil.location * MyCFG.edge) list * MyCFG.node -> (MyCFG.node * S.C.t * commitID->S.D.t) -> (MyCFG.node * S.C.t  -> S.D.t -> unit) -> (Cil.varinfo -> G.t) -> (Cil.varinfo -> G.t -> unit) -> D.t
  end
=
struct
  type lv = MyCFG.node * S.C.t * commitID
  type gv = varinfo
  type ld = S.D.t
  type gd = S.G.t
  ...
  let common_ctx var pval (getl:lv -> ld) sidel getg sideg : (D.t, G.t) ctx * D.t list ref =
    ...
    let rec ctx =
      ...
      ; spawn   = (fun f d -> let c = S.context d in
                    ...
                    ignore (getl (Function f, c, "COMMITID") ))
      ; split   = (fun (d:D.t) _ _ -> r := d::!r)
      ; sideg   = sideg
      ; assign = (fun ?name _    -> failwith "Cannot \"assign\" in common context.")
      }
  ...
  let tf_normal_call ctx lv e f args  (getl: lv ->ld) sidel getg sideg =
    let combine (cd, fd) = S.combine {ctx with local = cd} lv e f args fd in
    let paths = S.enter ctx lv f args in
    let _     = if not full_context then List.iter (fun (c,v) -> if not (S.D.is_bot v) then sidel (FunctionEntry f, S.context v) v) paths in
    let paths = List.map (fun (c,v) -> (c, if S.D.is_bot v then v else getl (Function f, S.context v, "COMMIT_ID"))) paths in
    let paths = List.filter (fun (c,v) -> not (D.is_bot v)) paths in
    let paths = List.map combine paths in
    List.fold_left D.join (D.bot ()) paths
  ...
  let tf (v,c) (edges, u) (getl: lv -> ld) sidel getg sideg =
    let pval = getl (u,c, "COMMIT_ID") in
    let _, locs = List.fold_right (fun (f,e) (t,xs) -> f, (f,t)::xs) edges (getLoc v,[]) in
    List.fold_left2 (|>) pval (List.map (tf (v,Obj.repr (fun () -> c)) getl sidel getg sideg) edges) locs
  ...
  let tf (v,c) (e,u) getl sidel getg sideg =
    ...
    let d       = try tf (v,c) (e,u) getl sidel getg sideg
      with M.StopTheWorld -> D.bot ()
         | M.Bailure s -> Messages.warn_each s; (getl (u,c, "COMMIT_ID"))  in
    let _       = current_node := old_node in
    d

  let system (v,c) =
    print_string (S.C.short 30 c);
    match v with
    | FunctionEntry _ when full_context ->
      [fun _ _ _ _ -> S.val_of c]
