...
struct
  ...
  let rec top_value ?(varAttr=[]) (t: typ) (uk:AddressDomain.unknownKind): t =
    ...
    | TArray (ai, length, _) ->
      ...
      Array (CArrays.make ~varAttr ~typAttr len (top_value ai uk))
    ...
    let one_addr =
      ...
      | Addr.Addr ({ vtype; _} as v, o) as a ->
        begin match Cil.unrollType vtype, Cil.unrollType t with
          ...
          | TFloat _, _
          | _, TFloat _ -> UnknownPtr {node = !Node.current_node; kind = Cast}
  ...
  *)
  ...
  let rec join x y =
    ...
    | (Int x, Address y)
    | (Address y, Int x) -> Address (AD.join y (AD.of_int ~uk:Join x))
  ...
  let widen x y =
    ...
    | (Int x, Address y)
    | (Address y, Int x) -> Address (AD.widen y (AD.join y (AD.of_int ~uk:Join x)))
  ...
  let rec meet x y =
    ...
    | (Address x, Int y) -> Address (AD.meet x (AD.of_int ~uk:Join y)) (* TODO: should Meet be an unknownKind? *)
  ...
  let rec narrow x y =
    ...
    | (Address x, Int y) -> Address (AD.narrow x (AD.of_int ~uk:Join y)) (* TODO: should Narrow be an unknownKind? *)
  ...
  let rec invalidate_abstract_value = function
    ...
    | Address _ -> Address (AD.top_ptr Invalidate)
