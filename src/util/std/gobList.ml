...
let rec fold_left3_seq f acc l1 l2 s3 = match l1, l2, Seq.uncons s3 with
  | [], [], None -> acc
  | x1 :: l1, x2 :: l2, Some(x3, s3) -> fold_left3_seq f (f acc x1 x2 x3) l1 l2 s3
  | _, _, _ -> invalid_arg "GobList.fold_left3_seq"
...
(** Like [for_all3], but the third argument is a sequence. *)
let rec for_all3_seq f l1 l2 s3 = match l1, l2, Seq.uncons s3 with
  | [], [], None -> true
  | x1 :: l1, x2 :: l2, Some (x3, s3) ->
    f x1 x2 x3 && (for_all3_seq [@tailcall]) f l1 l2 s3
  | _, _, _ -> invalid_arg "GobList.for_all3_seq"
