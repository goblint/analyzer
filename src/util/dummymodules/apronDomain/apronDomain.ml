...
struct
  ...
  let eval_cil_exp _ _ = raise_error ()
  let eval_assert_cil_exp _ _ = raise_error ()
  let assign_int_value _ _ = raise_error ()
...
=
struct
  type t = ApronDomain.t * MapDomain.MapTop(Lattice.Prod(Basetype.Strings)(Basetype.Strings))(EquationField).t
