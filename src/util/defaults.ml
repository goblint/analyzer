...
let _ = ()
      ...
      ; reg Experimental "exp.side-entries"      "true"  "Side-effect the start values to functions. Required for partial contexts (or ana.ctx_insens). If set to false, initial local values for start points are generated from the context via val_of: C.t -> D.t."
