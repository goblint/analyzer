...
let goblint_descs_list: (string * LibraryDesc.t) list = LibraryDsl.[
    ...
    ("__goblint_check_inner", special [__ "exp" []; drop "expstr" []] @@ fun exp -> Assert { exp = stripOuterBoolCast exp; check = true; refine = false });
    ("__goblint_assume_inner", special [__ "exp" []; drop "expstr" []] @@ fun exp -> Assert { exp = stripOuterBoolCast exp; check = false; refine = true });
    ("__goblint_assert_inner", special [__ "exp" []; drop "expstr" []] @@ fun exp -> Assert { exp = stripOuterBoolCast exp; check = true; refine = get_bool "sem.assert.refine" });
