...
(* stores the configurations for functions *)
let function_config : (Cil.fundec, (bool * bool * bool * bool)) Hashtbl.t = Hashtbl.create 500
let changed_configs = ref false
...
let precision_from_node (): precision =
  ...
  if Option.is_some node then
    ...
    let fundec_prec = precision_from_fundec fd in
    let improved_prec = Hashtbl.find_opt function_config fd in
    match improved_prec with
    | Some p -> p
    | None -> fundec_prec
  else
