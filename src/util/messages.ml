...
let msg_group_race_old group_name errors =
  let m = Message.{tags = [Category Race]; severity = Warning; multipiece = Group {group_text = group_name; pieces = List.map (fun (s, loc) -> Piece.{loc = Some loc; text = s; context = None}) errors}} in
  add m;
