...
and eq_exp_acc2 ?(no_const_vals = false) ~(acc: (typ * typ) list) ~(rename_mapping: rename_mapping) (a: exp) (b: exp)  = eq_exp a b ~rename_mapping ~acc

and eq_exp ?(no_const_vals = false) (a: exp) (b: exp) ~(rename_mapping: rename_mapping) ~(acc: (typ * typ) list) =
  match a, b with
  | Const c1, Const c2 ->  if no_const_vals then true else eq_constant c1 c2 ~rename_mapping ~acc
...
let eq_instr (a: instr) (b: instr) ~(rename_mapping: rename_mapping) =
  ...
  let comp_args ~rename_mapping f1 args1 args2 = match f1 with
    | Lval (Var v1,_) when List.mem v1.vname skip_const_args_funs ->
      GobList.equal (eq_exp ~rename_mapping ~no_const_vals:true ) args1 args2
    | _ -> GobList.equal (eq_exp ~rename_mapping) args1 args2
  in
  match a, b with
  ...
  | VarDecl (v1, _l1), VarDecl (v2, _l2) -> eq_varinfo ~rename_mapping v1 v2
  | _, _ -> false
