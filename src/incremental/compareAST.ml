...
and eq_typ_acc (a: typ) (b: typ) (acc: (typ * typ) list) (rename_mapping: rename_mapping) : bool * rename_mapping =
  ...
  let r, updated_rename_mapping = match a, b with
    ...
    | TFun (typ1, (Some list1), varArg1, attr1), TFun (typ2, (Some list2), varArg2, attr2) ->
      ...
      forward_list_equal (eq_args acc) list1 list2 &&>
      (varArg1 = varArg2) &&>>
    ...
    | TEnum (enuminfo1, attr1), TEnum (enuminfo2, attr2) ->
      ...
      (if res && enuminfo1.ename <> enuminfo2.ename then enuminfo2.ename <- enuminfo1.ename);
      res, rm
...
and eq_typ (a: typ) (b: typ) (rename_mapping: rename_mapping) : bool * rename_mapping = eq_typ_acc a b [] rename_mapping
...
and eq_args (acc: (typ * typ) list) (a: string * typ * attributes) (b: string * typ * attributes) (rename_mapping: rename_mapping) : bool * rename_mapping = match a, b with
    (name1, typ1, attr1), (name2, typ2, attr2) ->
    (rename_mapping_aware_name_comparison name1 name2 rename_mapping, rename_mapping) &&>>
...
and eq_varinfo (a: varinfo) (b: varinfo) (rename_mapping: rename_mapping) : bool * rename_mapping =
  ...
  let (locals_renames, method_rename_mappings) = rename_mapping in

  (*When we compare function names, we can directly compare the naming from the rename_mapping if it exists.*)
  let isNamingOk, updated_method_rename_mappings = match a.vtype, b.vtype with
    | TFun(_, aParamSpec, _, _), TFun(_, bParamSpec, _, _) -> (
        ...
        | Some method_rename_mapping ->
          ...
          is_naming_ok, method_rename_mappings
        | None ->
          if a.vname <> b.vname then
            (*Function that extracts the names from the param spec of the TFun*)
            let extract_names_from_params param_spec =
              Option.map (fun list -> List.map (fun (name, _, _) -> name) list) param_spec |>
              Option.value ~default:[]
            in

            (*No mapping exists yet. Create one.*)
            let aParamNames = extract_names_from_params aParamSpec in
            let bParamNames = extract_names_from_params bParamSpec in
            ...
            true, StringMap.add a.vname assumption method_rename_mappings
          else true, method_rename_mappings
      )
    | _, _ -> rename_mapping_aware_name_comparison a.vname b.vname rename_mapping, method_rename_mappings
  in
  ...
  let typ_rename_mapping = match b.vtype with
    | TFun(_, _, _, _) -> (
        ...
        | None -> (StringMap.empty, updated_method_rename_mappings)
      )
    | _ -> (locals_renames, updated_method_rename_mappings)
  in
  ...
  (typeCheck, (locals_renames, updated_method_rename_mappings)) &&>>
