...
struct 
  ...
  struct
    open C
    module DR = Lattice.Reverse (D)
    module ND = Lattice.Prod (D) (DR)
...
(*      ignore (printf "considering %a\n" V.pretty_trace x);*)
      ...
      let update_rhs_value i x d = 
        ...
        if not (D.equal d oldv) then begin
          VMap.replace sigmaw x (IMap.add i (GU.joinvalue D.join oldv d) oldm);
          true
        end else 
          false
      in
...
struct
  ...
  let solve (system: system) (initialvars: variable list) (start:(Var.t * VDom.t) list): solution' = 
    ...
    let f (b,k) = function 
      | `Left d -> ()
      | `Right (_,d) -> if b then VMap.add lm k d
    in
    Sol.VMap.iter f map;
    lm, gm
