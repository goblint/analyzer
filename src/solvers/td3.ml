    ...
    let solve box st vs data =
      ...
      if !incremental_mode = "incremental" then (
        ...
        let add_nodes_of_fun (functions: fundec list) (nodes)=
          ...
          in
          List.iter (fun f -> Hashtbl.replace nodes (Node.show_id (FunctionEntry f)) (); Hashtbl.replace nodes (Node.show_id (Function f)) (); add_stmts f; Hashtbl.replace nodes (string_of_int (f.svar.vid + 10_000_000_000)) ()) functions;
        in
