...
module JoinContr (Sol: GenericEqBoxSolver) =
  ...
  functor (HM:Hash.H with type key = S.v) ->
  struct
    include Sol (Generic_arinc.SimpleSysConverter (S)) (HM)
  end
...
module MoreVars (Sol: GenericEqBoxSolver) =
  ...
  functor (HM:Hash.H with type key = S.v) ->
  struct
    module HM1   = Hashtbl.Make (Generic_arinc.ExtendInt (S.Var))
    module EqSys = Generic_arinc.NormalSysConverter (S)
...
let _ =
  ...
  Selector_arinc.add_solver ("widen1",  (module W1 : GenericGlobSolver));
  Selector_arinc.add_solver ("widen2",  (module W2 : GenericGlobSolver));
  Selector_arinc.add_solver ("widen3",  (module W3 : GenericGlobSolver));
  let module S2 = GlobSolverFromIneqSolver (JoinContr (TwoPhased (struct let ver = 1 end))) in
  Selector_arinc.add_solver ("two",  (module S2 : GenericGlobSolver));
  let module S1 = GlobSolverFromIneqSolver (JoinContr (Make (struct let ver = 1 end))) in
  Selector_arinc.add_solver ("new",  (module S1 : GenericGlobSolver));
  Selector_arinc.add_solver ("slr+", (module S1 : GenericGlobSolver))
...
let _ =
  ...
  Selector_arinc.add_solver ("slr1", (module S1 : GenericGlobSolver)); (* W&N at every program point *)
  Selector_arinc.add_solver ("slr2", (module S2 : GenericGlobSolver)); (* W&N dynamic at certain points, growing number of W-points *)
  Selector_arinc.add_solver ("slr3", (module S3 : GenericGlobSolver)); (* same as S2 but number of W-points may also shrink *)
  Selector_arinc.add_solver ("slr4", (module S4 : GenericGlobSolver)); (* restarting: set influenced variables to bot and start up-iteration instead of narrowing *)
  ...
  Selector_arinc.add_solver ("slr1p", (module S1p : GenericGlobSolver)); (* same as S1-4 above but with side-effects *)
  Selector_arinc.add_solver ("slr2p", (module S2p : GenericGlobSolver));
  Selector_arinc.add_solver ("slr3p", (module S3p : GenericGlobSolver));
  Selector_arinc.add_solver ("slr4p", (module S4p : GenericGlobSolver));
  ...
  Selector_arinc.add_solver ("slr1x", (module S1 : GenericGlobSolver)); (* alias for S1-4 *)
  Selector_arinc.add_solver ("slr2x", (module S2 : GenericGlobSolver));
  Selector_arinc.add_solver ("slr3x", (module S3 : GenericGlobSolver));
  Selector_arinc.add_solver ("slr4x", (module S4 : GenericGlobSolver));
  ...
  Selector_arinc.add_solver ("slr1xp", (module S1p : GenericGlobSolver)); (* alias for S1p-4p *)
  Selector_arinc.add_solver ("slr2xp", (module S2p : GenericGlobSolver));
  Selector_arinc.add_solver ("slr3xp", (module S3p : GenericGlobSolver));
  Selector_arinc.add_solver ("slr4xp", (module S4p : GenericGlobSolver));
