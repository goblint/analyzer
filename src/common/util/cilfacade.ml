...
let getFuns fileAST : startfuns =
  ...
  let f acc glob =
    ...
    | GFun({svar={vname=mn; vattr=attr; vtype= TFun (t, _, _, _); _}; _} as def, _) when Cil.hasAttribute "goblint_caml_prim" attr || Cil.hasAttribute "goblint_caml_prim" (Cil.typeAttrs(t)) -> add_main def acc
    | _ -> acc
  in
