...
let check_arguments () =
  ...
  if partial_context && not (get_bool "exp.side-entries") then fail "exp.side-entries can't be off with partial contexts (exp.addr-context, exp.no-int.context, exp.no-interval-context)";
  let ctx_insens = Set.(cardinal (intersect (of_list (get_list "ana.ctx_insens")) (of_list (get_list "ana.activated")))) > 0 in
  if ctx_insens && not (get_bool "exp.side-entries") then warn "Disabling exp.side-entries might lead to exceptions (undef. operations on top) with context-insensitive analyses enabled (ana.ctx_insens)";
