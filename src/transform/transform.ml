...
module PartialEval = struct
  ...
  class visitor ask = object
    ...
    method! vstmt s =
      ...
      (* Fmt.pr "Set loc at stmt %a to %a\n" d_stmt s CilType.Location.pp !loc; *)
      DoChildren
    method! vexpr e =
      let eval e = match (ask !loc).Queries.f (Queries.EvalInt e) with
        | x when Queries.ID.is_int x ->
          ...
          Fmt.pr "Replacing non-constant expression %a with %a at %a\n" d_exp e d_exp e' CilType.Location.pp !loc;
          e'
        | _ ->
          Fmt.pr "Can't replace expression %a at %a\n" d_exp e CilType.Location.pp !loc; e
      in
      match e with
