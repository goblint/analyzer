...
cfile_loop1="tests/regression/75-termination/02-simple-loop-nonterminating.c"      
...
# run analysis, write cil output to file and enable visualization via html
./goblint $cfile_loop1 $options_term --enable justcil > output.txt
./goblint $cfile_loop1 $options_term --html 

# set up server to see visualizatino
python3 -m http.server --directory result 8082
