thread creation prevents disabling of locked creation analyses:
  $ goblint --set dbg.level debug --enable ana.autotune.enabled --set ana.autotune.activated[+] reduceAnalyses --set ana.activated[+] threadJoins --set ana.activated[+] threadDescendants --set ana.activated[+] mustlockHistory --set ana.activated[+] descendantLockset --set ana.activated[+] creationLockset 42-dl_cl_simple_racefree.c 2>&1 | grep -E -c "Activated analyses: [[:print:]]*threadDescendants, mustlockHistory, descendantLockset, creationLockset"
  1
