// CRAM
// large image manifest: larger benchmark body with few phase boundaries; intended for level01.
#include <pthread.h>
...
int main(void) {
  ...
  pthread_join(p, 0);
  pthread_join(a, 0);
  if (summary.accepted != 151 || summary.rejected != 12 || summary.checksum != 23) {
    reach_error();
    abort();
  }
  return 0;
