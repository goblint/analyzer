// CRAM
// large backup catalog: larger benchmark body with few phase boundaries; intended for level01.
#include <pthread.h>
...
void *planner(void *arg) {
  ...
  /* GHOST planner 1 */ summary.accepted += score + 8;
...
void *auditor(void *arg) {
  ...
  pthread_mutex_lock(&summary_lock);
  /* GHOST auditor 1 */ summary.rejected += rejected + 3;
  pthread_mutex_unlock(&summary_lock);
  pthread_mutex_lock(&summary_lock);
...
int main(void) {
  ...
  pthread_join(p, 0);
  pthread_join(a, 0);
  if (summary.accepted != 153 || summary.rejected != 8 || summary.checksum != 21) {
    reach_error();
    abort();
  }
  return 0;
