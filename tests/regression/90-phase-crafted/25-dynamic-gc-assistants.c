// CRAM
// dynamic gc assistants: relevant phase workers plus dynamically spawned ghost-free background workers.
#include <pthread.h>
...
int main(void) {
  ...
  pthread_join(p, 0);
  pthread_join(c, 0);
  if (state.a != 58 || state.b != 71 || state.tag != 5) {
    reach_error();
    abort();
  }
  return 0;
