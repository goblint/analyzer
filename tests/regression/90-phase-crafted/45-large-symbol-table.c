// CRAM
// large symbol table: larger benchmark body with few phase boundaries; intended for level00.
#include <pthread.h>
...
int main(void) {
  ...
  pthread_join(p, 0);
  pthread_join(a, 0);
  if (summary.accepted != 157 || summary.rejected != 4 || summary.checksum != 20) {
    reach_error();
    abort();
  }
  return 0;
