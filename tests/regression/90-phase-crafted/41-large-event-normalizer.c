// CRAM
// large event normalizer: larger benchmark body with few phase boundaries; intended for level00.
#include <pthread.h>
...
void *planner(void *arg) {
  ...
  /* GHOST planner 1 */ summary.accepted += score + 9;
...
int main(void) {
  ...
  pthread_join(p, 0);
  pthread_join(a, 0);
  if (summary.accepted != 155 || summary.rejected != 10 || summary.checksum != 20) {
    reach_error();
    abort();
  }
  return 0;
