// CRAM
// large metrics fold: larger benchmark body with few phase boundaries; intended for level00.
#include <pthread.h>
...
int main(void) {
  ...
  pthread_join(p, 0);
  pthread_join(a, 0);
  if (summary.accepted != 153 || summary.rejected != 14 || summary.checksum != 22) {
    reach_error();
    abort();
  }
  return 0;
