// CRAM
// large retention policy: larger benchmark body with few phase boundaries; intended for level01.
#include <pthread.h>
...
int main(void) {
  ...
  pthread_join(p, 0);
  pthread_join(a, 0);
  if (summary.accepted != 155 || summary.rejected != 16 || summary.checksum != 21) {
    reach_error();
    abort();
  }
  return 0;
