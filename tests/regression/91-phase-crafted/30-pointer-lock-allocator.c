// CRAM
// pointer lock allocator: workers lock and unlock mutexes through pointer variables.
#include <pthread.h>
...
void *left_worker(void *arg) {
  ...
  pthread_mutex_lock(lockp);
  /* GHOST left_worker 2 */ record.stamp |= 4;
  pthread_mutex_unlock(lockp);
  return 0;
}
...
int main(void) {
  ...
  pthread_join(l, 0);
  pthread_join(r, 0);
  if (!(record.x == 42 && record.y == 56 && record.stamp != 0)) {
    reach_error();
    abort();
  }
  return 0;
