// CRAM
// large report rollup: larger benchmark body with few phase boundaries; intended for level00.
#include <pthread.h>
...
void *planner(void *arg) {
  ...
  /* GHOST planner 1 */ summary.accepted += score + 5;
...
void *auditor(void *arg) {
  ...
  int rejected = 1;
  pthread_mutex_lock(&summary_lock);
  /* GHOST auditor 1 */ summary.rejected += rejected + 5;
  pthread_mutex_unlock(&summary_lock);
  pthread_mutex_lock(&summary_lock);
...
int main(void) {
  ...
  pthread_join(p, 0);
  pthread_join(a, 0);
  if (summary.accepted != 147 || summary.rejected != 7 || summary.checksum != 20) {
    reach_error();
    abort();
  }
  return 0;
