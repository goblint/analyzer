// CRAM
// pointer lock router: workers lock and unlock mutexes through pointer variables.
#include <pthread.h>
...
void *left_worker(void *arg) {
  ...
  pthread_mutex_lock(lockp);
  /* GHOST left_worker 2 */ record.stamp |= 8;
  pthread_mutex_unlock(lockp);
  return 0;
}
...
int main(void) {
  ...
  pthread_join(l, 0);
  pthread_join(r, 0);
  if (!(record.x == 41 && record.y == 55 && record.stamp != 0)) {
    reach_error();
    abort();
  }
  return 0;
