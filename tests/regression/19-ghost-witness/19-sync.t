Does not succeed without witness
  ...
  $ goblint --enable warn.deterministic --conf ../../../conf/svcomp26/common.json --conf ../../../conf/svcomp26/verify.json --conf ../../../conf/svcomp26/level04.json --sets ana.specification "CHECK( init(main()), LTL(G ! call(reach_error())) )" --enable ana.sv-comp.functions  --set ana.base.privatization protection-atomic-ghost --sets exp.architecture 64bit 19-sync.c --disable witness.yaml.enabled
...
Run with the second witness, which additionally contains invariants
  ...
  $ goblint --enable warn.deterministic --conf ../../../conf/svcomp26/common.json --conf ../../../conf/svcomp26/verify.json --conf ../../../conf/svcomp26/level04.json --set witness.yaml.invariant-types[+] location_invariant --set ana.path_sens[+] phaseGhostSplit --set ana.specification "CHECK( init(main()), LTL(G ! call(reach_error())) )" --enable ana.sv-comp.functions --set ana.base.privatization protection-atomic-ghost --set exp.architecture 64bit --set witness.yaml.validate 19-sync-invariants.yml --set ana.activated[+] phaseGhost --set ana.activated[+] phaseGhostSplit --disable witness.yaml.enabled 19-sync.c
  [Info] Enabled congruence domain.
  [Info] Enabled widening thresholds
  [Info] Enabled octagon domain ONLY for:
  [Info] data1, data2, i, count, tmp, count, i, j, i___0, j___0, k, size, r, expression
...
Run with the second witness, which additionally contains invariants and unassume them
  ...
  [Info][Witness] phaseGhost: global ghost_thread1_phase is only accessed by unique thread [main, thread1@19-sync.c:45:3-45:37] and is monotonically increased to known bounds
  [Info][Witness] phaseGhost: global ghost_thread2_phase is only accessed by unique thread [main, thread2@19-sync.c:46:3-46:37] and is monotonically increased to known bounds
  [Info][Witness] unassume invariant: (data1 == 16 && data2 == 5) && (ghost_thread1_phase == 2 && ghost_thread2_phase == 2) (19-sync.c:48:3-48:22)
  [Info][Witness] unassume invariant: (data1 == 16 && data2 == 5) && (ghost_thread1_phase == 2 && ghost_thread2_phase == 2) (19-sync.c:49:7-49:28)
  [Info][Witness] witness validation summary:
    confirmed: 2
    unconfirmed: 0
    refuted: 0
