...
(subdir %s
 (rule
  ...
  (action
   (progn
    (ignore-outputs
     (run goblint --conf svcomp21.json --set ana.specification %%{prop} %%{c} --enable witness.graphml.uncil --enable exp.arg.enabled --set exp.arg.dot.path arg.dot --set exp.arg.dot.node-label empty))
