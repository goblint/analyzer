#!/usr/bin/ruby
#Requires parallel: sudo gem install parallel
...
#   solfile = File.join(testresults, p.name + ".sol.txt")
  ...
  `#{goblint} #{filename} --set keepcpp true --set justcil true #{p.params} >#{cilfile} 2> /dev/null`
#   p.size = `wc -l #{cilfile}`.split[0]
  starttime = Time.now
  cmd = "#{goblint} #{filename} #{p.params} 1>#{warnfile} --set keepcpp true --set printstats true  2>#{statsfile}"
  system(cmd)
  endtime   = Time.now
  status = $?.exitstatus
#   `#{goblint} #{filename} #{p.params} --trace con 2>#{confile}` if tracing
#   `#{goblint} #{filename} #{p.params} --trace sol 2>#{solfile}` if tracing
  if status == 0 then print '.' else print 'X' end
...
File.open(theresultfile, "w") do |f|
  ...
  projects.each do |p|
    ...
    if p.group != gname then
      gname = p.group
      headings = ["ID", "Name", "CIL file", "Checks", "Time", "Problems"]
...
puts "  Single: ./scripts/pupdate_suite.rb simple_rc"
puts "  Groups: ./scripts/pupdate_suite.rb group mutex"
puts "  Future: ./scripts/pupdate_suite.rb future"
